...
In the new manuscript only two pages are dedicated to the description of the algorithm.  An important reason for describing the algorithm is that much of the notation used during the analysis of the estimator is most naturally introduced during the description of the algorithm.  For example, the proof of Lemma~\zref{lem:diffatlambdaL} makes use of the notation $e_i$ and the function $LS(\{d_i, i \in D\})$ introduced during the description of the algorithm.  To qualify this for the reader we have included the following sentences at the beginning of Section~\zref{sec:least-squar-estim}:
...
\item\label{R1it3a}\textbf{Comment:} 
	...
	\\\textbf{Response:}
	We agree with the reviewer.  In the updated manuscript, we refer to an estimator that does not use pilot symbols as \emph{non-data-aided}.
\item\textbf{Comment:} 
	missing `,' after (5)
...
\item\textbf{Comment:} 
	...
	\\\textbf{Response:} This typographical error has been corrected.
\item\textbf{Comment:} 
...
the LS results are compared to an alternative solution represented by the Viterbi and Viterbi (VV) algorithm. Why not doing the same with $|P| > 0$ by the following approach, which is definitely much simpler from the computational point of view (complexity in the order $O(L)$ instead of $O(L \log(L))$ required by LS) and the jitter performance will probably differ not very much from that of the LS variant (in Figs.~1 to 3 this can be seen for $|P| = 0$):
	...
	\end{itemize}
	\textbf{Response:}
The non-data-aided estimation of SNR is a significant difficulty when implementing the approach suggested by the reviewer.  As observed by simulation in~\cite{Pauluzzi2000} known non-data-aided estimators of SNR appear to be biased.  The bias is often small when the noise variance is small, but typically becomes significant as the noise variance increases.  This makes it difficult to estimate SNR so as to know when to use a DA algorithm with only the available pilots and when to use the VV algorithm.  Analysis of the bias of the chosen non-data-aided SNR estimator is required in order to justify this approach.  To date the bias and variance of non-data-aided estimators of SNR (or equivalently carrier amplitude) appear to have been measured only by Monte-Carlo simulation~\cite{Pauluzzi2000}.  In the abstract of our new paper we point out that
...
\item\textbf{Comment:} \label{rev2:biasmse}
...
We do agree with the reviewer this this plot by itself initially appears misleading.  To alleviate this confusion in the new manuscript we instead plot the average square error computed in the usual way by Monte-Carlo simulation and the solid line is given by the bias plus variance squared.  The following paragraph has also been included in the simulations section (Section~\zref{sec:simulations}) in the new manuscript. 
...
\item\textbf{Comment:} \label{rev2:turbosync}
...
The major contribution of this paper is the description of the asymptotic properties of the least squares estimator for the phase and amplitude. Although it seems technically correct, the authors do not make clear the relevance of their work to signal processing applications and to timely problems. They limit themselves in presenting several lemmas and proofs. Therefore, I cannot recommend this manuscript for publication in the IEEE Transactions on Signal Processing.
	\\\textbf{Response:}
We have greatly expanded the introduction of the manuscript to make the practical and theoretical importance of our of results clear.  Also see our response to comment~\ref{rev4:shouldbettermotivate} of reviewer~4.
\end{enumerate}
...
\begin{enumerate}
\item\textbf{Comment:} \label{rev4:shouldbettermotivate}
...
\item\textbf{Comment:} 
	...
	\\\textbf{Response:}
We have included simulations with turbo synchronisation proceedures.  See our response to comment~\ref{rev2:turbosync} of reviewer~2.
...
\item\textbf{Comment:} 
	...
	\\\textbf{Response:} 
Please see response~\ref{rev2:biasmse} to reviewer~2.
...
The original Section~VII describing the Gaussian noise case has been removed from the revised manuscript.
