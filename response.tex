...
We have shortened Section~\zref{sec:least-squar-estim} but have left the description of the algorithm intact.  The primary reason for this is that much of the notation used during the analysis of the estimator is most naturally introduced during the description of the algorithm.  To qualify this for the reader we have included the following sentences at the beginning of Section~\zref{sec:least-squar-estim}.

``
In this section we derive Mackentun's algorithm to compute the least squares estimators of the carrier phase and amplitude~\cite{Mackenthun1994}.  Mackenthun specifically considered the non-data aided setting, so we modify the algorithm to include pilot symbols.  Although the modification is straightforward, much of the notation that we will use for analysing the estimator in Section~\ref{sec:stat-prop-least} is naturally introduced during the description of the algorithm.  For this reason, and so that our paper is self contained, we describe the algorithm in full.
''
...
the LS results are compared to an alternative solution represented by the Viterbi and Viterbi (VV) algorithm. Why not doing the same with $|P| > 0$ by the following approach, which is definitely much simpler from the computational point of view (complexity in the order $O(L)$ instead of $O(L \log(L))$ required by LS) and the jitter performance will probably differ not very much from that of the LS variant (in Figs.\ 1Ð3 this can be seen for $|P| = 0$):
	...
	\end{itemize}
	\textbf{Response:}
The non data-aided estimation of SNR is a significant difficulty when implementing this approach.  As observed by simulation in [c] known non data-aided estimators of SNR appear to be biased.  The bias is often small when the noise variance is small, but typically becomes significant as the noise variance increases.  This makes it difficult to estimate SNR so as to know when to use a DA algorithm with only the available pilots and when to use the VV algorithm.  Analysis of the bias of the chosen non data-aided SNR estimator is required in order to justify this approach.  To date the bias and variance of non data-aided estimators of SNR (or equivalently carrier amplitude) appear to have been measured only by Monte-Carlo simulation [c].  In the abstract of our new paper we point out that our of paper
...
\item\textbf{Comment:} 
	...
	\\\textbf{Response:}
We thank their reviewer for the comments. We have expanded the introduction of the manuscript to make the practical and theoretical importance of our of results clear.
\item\textbf{Comment:} 
...
% In view of this, a direct comparison with a turbo synchronisation proceedure is not meaningful. A meaninful simulation that could be implemented is to compare the results of turbo synchronisation with different initialisers.  For example one could initialise the proceedure using only the pilots and a data-aided algorithm and also with our least squares algorithm and compare the bit error rates.  It can be reasonably expected that the initialiser that is a more accurate estimator of carrier phase and amplitude with also be a better initialiser for a turbo synchronisation proceedure and will thus have a smaller bit error rate. So, for example, we expect our least square estimator to be a beter initialiser at moderate and high SNR and a data-aided approach to be better at low SNR provided there are sufficiently many pilots symbols.
...
The original Sec.\ VII has been removed from the revised manuscript.
