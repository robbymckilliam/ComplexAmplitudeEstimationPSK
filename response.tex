...
The description of the algorithm has been left intact.  An important reason for describing the algorithm is that much of the notation used during the analysis of the estimator is most naturally introduced during the description of the algorithm.  For example, the proof of Lemma~\zref{lem:fracpartlambdahatnotpi} makes use of the notation $e_i$ and the function $LS(\{d_i, i \in D\})$ introduced during the description of the algorithm.  To qualify this for the reader we have included the following sentences at the beginning of Section~\zref{sec:least-squar-estim}:
...
\item\label{R1it2}\textbf{Comment:} 
	...
	\\\textbf{Response:}	
	For improved readability of the manuscript, we have moved all of the lemmas into the appendix. We have also added a discussion of the implications and the practical relevance of the theorems after the statement of Theorems~\zref{thm:consistency} and~\zref{thm:normality} in Section~\zref{sec:stat-prop-least}.  The introduction has been expanded to make the practical and theoretical importance of our of results clear.
...
We have included CRBs for figures in the simulations section.  The following sentences have been included:

``Figures~\zref{fig:plotphaseBPSK} and~\zref{fig:plotphaseQPSK} display the non-data aided Cram\'{e}r-Rao lower bound (CRB) for carrier phase given in~\cite{Cowley_crbs_phase_freq_1996}.  These CRBs are derived for the non-data aided setting and are only valid for estimators that do not make use of pilots symbols, i.e., $\abs{P}=\emptyset$.''
...
Thus, the techniques in our paper can potentially be deployed to accurately model the bias and variance of a non-data-aided SNR estimator in order to implement the reviewer's proposal.  We confirm, however, that analysing and implementing the reviewer's suggested technique is beyond the scope of the present paper which places primary interest in analysing the least squares estimator.
...
\item\textbf{Comment:} \label{rev2:turbosync}
	...
	\\\textbf{Response:}
	We have added the following paragraphs to the introduction to place our results in context with exiting turbo synchronisation literature.
...
The major contribution of this paper is the description of the asymptotic properties of the least squares estimator for the phase and amplitude. Although it seems technically correct, the authors do not make clear the relevance of their work to signal processing applications and to timely problems. They limit themselves in presenting several lemmas and proofs. Therefore, I cannot recommend this manuscript for publication in the IEEE Transactions on Signal Processing.
	\\\textbf{Response:}
We have greatly expanded the introduction of the manuscript to make the practical and theoretical importance of our results clear.  Also see our response to comment~\ref{rev4:shouldbettermotivate} of reviewer~4.
\end{enumerate}
...
\item\textbf{Comment:}
	...
	\\\textbf{Response:}
We have greatly expanded the introduction of the manuscript to make the practical and theoretical importance of our of results clear.
...
\item\textbf{Comment:} 
	...
	\\\textbf{Response:}
See our response to comment~\ref{rev2:turbosync} of reviewer~2.
...
\item\textbf{Comment:} \label{R4it2e}
	...
	\\\textbf{Response:}
The original Section~7 describing the Gaussian noise case has been removed from the revised manuscript.  The original Section~3 has been removed.  Necessary results about circular random variables are now introduced as needed.
%We agree with the reviewer that circularly symmetric random variables (described in Section~3 of the original manuscript) are well known, but point out that  section specifically discussed random variables of the form $Re^{j\Phi}=1+W$, where $W$ is circularly symmetric. The joint pdf of $R$ and $\Phi$ is a crucial ingredient for the asymptotic analysis presented in the manuscript and therefore, we feel that this material must be retained. However, we have modified the way that this material is introduced.  It is now introduced as needed when the asymptotic properties of the least squares estimator are derived.
Regarding Section~2 we refer the reviewer to our response to comment~\eqref{comment:shortersec2} of reviewer~1.
