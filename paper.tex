...
Let $X = Z e^{j\Theta}$ be a circularly symmetric complex random variable with continuous pdf $\tfrac{1}{2\pi}f_Z(z)$.  Let $R > 0$ and $\Phi \in [0, 2\pi)$ be random variables satisfying $R e^{j\Phi} = 1 + X$ and with joint pdf $f(r,\phi)$.  Then
\[
...
\end{lemma}
\begin{IEEEproof}
We only give a proof for $g_2$, the proof for $g$ being similar.  From 
\end{IEEEproof}
