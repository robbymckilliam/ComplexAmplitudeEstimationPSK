...
\begin{theorem}
Let $\{w_i\}$ be a sequence independent and identically distributed, zero mean, circularly symetric complex random variables.  Put $X_i = \rho_0 + w_i e^{-j\theta_0} s_i^*$ and $\Phi_i = \angle{X_i}$ and 
\[
h = \prob X_i e^{-j\round{\Phi_i}} = \prob X_1 e^{-j\round{\Phi_1}}.
\]
...
\] 
as $L \rightarrow \infty$ and each of $p$ and $d$ and $h$ are positive real numbers, then:
\begin{enumerate}
\item $\sfracpart{\theta_0 - \hat{\theta}}_\pi \rightarrow 0$ almost surely as $L \rightarrow \infty$,
\item $\hat{\rho} \rightarrow \rho_0 p + d h$ almost surely as $L \rightarrow \infty$,
\item Put $\lambda_L = \sfracpart{\theta_0 - \hat{\theta}}_\pi$ and $m_L = \hat{\rho} - \rho_0(p + dh)$.  Then the vector $[\sqrt{L}\lambda_L, \sqrt{L}m_L]$ converges to the bivariate normal distribution with zero mean and covariance
\[
C = ?
\]
