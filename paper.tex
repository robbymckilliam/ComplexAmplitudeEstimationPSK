...
\]
Put $\hat{\lambda}_L = \sfracpart{\theta_0 - \hat{\theta}}_\pi$ and $\hat{m}_L = \hat{\rho} - \rho_0 G(0)$. If $g(\phi)$ is continuous at $\phi = \tfrac{2\pi}{M}k+\tfrac{\pi}{M}$ for each $k = 0, 1, \dots M-1$, then the distribution of $(\sqrt{L}\lambda_L, \sqrt{L}m_L)$ converges to the bivariate normal with zero mean and covariance
\[
...
\end{equation}
Both $a_k(\lambda)$ and $b_k(\lambda)$ converge to zero as $\lambda$ goes to zero, whilst $d_k(\lambda)$ converges to 
\[
...
&= \lambda \left( D_k(\lambda)  + A_k(\lambda) + B_k(\lambda) \right).
\end{align*}
where 
\[
...
\]
\[
D_k(\lambda) = \sum_{k=0}^{M-1} d_k(\lambda).
\]
Both $A_k(\lambda)$ and $B_k(\lambda)$ converge to zero as $\lambda$ goes to zero, and $D_k(\lambda)$ converges to $H_1$ as $\lambda$ converges to zero.  Put
\[
\gamma(\lambda) = A_k(\lambda) + B_k(\lambda) + D_k(\lambda) - H_1.
\]
