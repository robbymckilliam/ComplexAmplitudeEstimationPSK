...
Before describing the statistical properties of the least squares estimator, we first require some properties of complex valued random variables.  A complex random variable $X$ is said to be \emph{circularly symmetric} if the distribution of its phase $\angle{X}$ is uniform on $[0,2\pi)$ and is independent of the distribution of its magnitude $\abs{X}$.  That is, if we define real random variables $Z \geq 0$ and $\Theta \in [0,2\pi)$ such that $Ze^{j\Theta} = X$, then $\Theta$ is uniformly distributed on $[0,2\pi)$ and is indepedent of $Z$.  If the probability density function of $Z$ is $f_Z(z)$, then the joint density function of $\Theta$ and $Z$ is 
\[
...
Let $X = Z e^{j\Theta}$ be a circularly symmetric complex random variable with pdf $\tfrac{1}{2\pi}f_Z(z)$.  Let $R > 0$ and $\Phi \in [0, 2\pi)$ be random variables satisfying $R e^{j\Phi} = 1 + X$.  Let $h_1(x)$ be the function
\[
...
\begin{lemma}
Let $X = Z e^{j\Theta}$ be a circularly symmetric complex random variable with pdf $\tfrac{1}{2\pi}f_Z(z)$ such that $z^{-1} f_Z(z)$ is nonincreasing with $z$.  Let $R > 0$ and $\Phi \in [0, 2\pi)$ be random variables satisfying $R e^{j\Phi} = 1 + X$.  Let $h_2$ and $k_2$ be the functions as,
\[
...
\]
where $\prob$ denotes probability.  Since $G(\lambda)$ is continuous and is uniquely maximised at $0$, then for any $\delta > 0$ there is a $\epsilon > 0$ such that if 
\[
\sabs{\hat{\lambda}_L} > \delta \qquad \text{then} \qquad \sabs{G(\hat{\lambda}_L) - G(0)} > \epsilon,
\]
...
It remains to prove part two of this theorem regarding the convergence of the least squares amplitude estimator $\hat{\rho}$.  From~\eqref{eq:hatrhoZ},
...
\]
where $o_P(1)$ denotes a number converging in probablity to zero as $L \rightarrow \infty$ and where the constant real number $p,d$ and $H$ are defined in the statement of Theorems~\ref{thm:consistency}~and~\ref{thm:normality}.  Lemma~\ref{lem:empiricprocc} shows that
\[
...
\]
Lemma~\ref{lem:convdistGLdash} shows that the distribution of $\sqrt{L}G_L'(0)$ converges to the normal with zero mean and variance $pA_1 + dA_2$ where $A_1$ and $A_2$ are real constant defined in the statement of Thereom~\ref{thm:normality}.  It follows that the distribution of $\sqrt{L}\hat{\lambda}_L$ converges to the normal with zero mean and variance
\[
\frac{pA_1 + dA_2}{(p + Hd)^2}.
\]
 
We now analyse the asymptotic distribution of $m_L = \hat{\rho} - \rho_0 G(0)$.  Define the function $T(\lambda) = \expect G_L(\lambda)$.  Using~\eqref{eq:rhoGLZ} we have
\begin{align*}
...
\end{align}
We analyse $k_1(\lambda)$ first.  By a Taylor expansion of $\sin(\lambda + \Phi_1)$ about $\Phi_1$ we obtain
\[
...
\end{lemma}
\begin{IEEEproof}
Since $\hat{\lambda}_L \in [-\tfrac{\pi}{M}, \tfrac{\pi}{M})$ it is only the behaviour of the function $k_2(\lambda)$ for $\lambda\in [-\tfrac{\pi}{M}, \tfrac{\pi}{M})$ that is relevant.  We will analyse $k_2(\lambda)$ for non negative $\lambda$, i.e., when $0 \leq \lambda < \tfrac{\pi}{M}$.  An analagous argument follows when $\lambda$ is negative, i.e., $-\tfrac{\pi}{M} \leq \lambda < 0$.  

To keep our notation clean put
\[
\psi_k = (2k+1)\tfrac{\pi}{M}.
\]
For each integer $k$, if $\Phi_1 \in [\psi_{k-1}, \psi_k)$, then
\[
\fracpart{\Phi_1} = \Phi_1 - \tfrac{2\pi k}{M}.
\]
Similarly if $\Phi_1 \in  [\psi_{k-1}, \psi_{k} - \lambda)$ then
\[
\fracpart{\Phi_1 + \lambda} = \Phi_1 + \lambda - \tfrac{2\pi k}{M},
\]
whilst if $\Phi_1 \in [\psi_{k} - \lambda, \psi_{k})$ then
\[
\fracpart{\Phi_1 + \lambda} = \Phi_1 + \lambda - \tfrac{2\pi (k+1)}{M}.
\]
So, when $\Phi_1 \in [\psi_{k-1}, \psi_{k} - \lambda )$,
\begin{align*}
\sin\fracpart{\lambda + \Phi_1} - &\sin\fracpart{\Phi_1} \\
&= \sin(\lambda + \Phi_1 - \tfrac{2\pi k}{M}) - \sin(\Phi_1 - \tfrac{2\pi k}{M}),
\end{align*}
and by a Taylor expansion of $\sin(\lambda + \Phi_1 - \tfrac{2\pi k}{M})$ about $\Phi_1 - \tfrac{2\pi k}{M}$ we obtain 
\begin{align*}
\sin\fracpart{\lambda + \Phi_1} - \sin\fracpart{\Phi_1} &= \lambda\big(\cos(\Phi_1 - \tfrac{2\pi k}{M}) + \zeta_k(\lambda)\big) \\
&= \lambda\big(\cos\fracpart{\Phi_1} + \zeta_k(\lambda)\big).
\end{align*}
where $\zeta_k(\lambda)$ is a function that goes to zero as $\lambda$ goes to zero.  Alternatively, when $\Phi_1 \in [ \psi_{k} - \lambda,\psi_{k})$,
\begin{align*}
\sin\fracpart{\lambda + \Phi_1} - &\sin\fracpart{\Phi_1} \\
&= \sin(\lambda + \Phi_1 - \tfrac{2\pi(k+1)}{M}) - \sin(\Phi_1 - \tfrac{2\pi k}{M})
\end{align*}
and by a Taylor expansion of $\sin(\lambda + \Phi_1 - \tfrac{2\pi(k+1)}{M})$ about $\Phi_1 - \tfrac{2\pi(k+1)}{M}$,
\begin{align*}
\sin&\fracpart{\lambda + \Phi_1} - \sin\fracpart{\Phi_1} \\
&=  s_k(\Phi_1) + \lambda\big(\cos(\Phi_1 - \tfrac{2\pi(k+1)}{M}) + \zeta_{k+1}(\lambda)\big) 
\end{align*}
where
\[
s_k(\Phi_1) = \sin(\Phi_1 - \tfrac{2\pi(k+1)}{M}) - \sin(\Phi_1 - \tfrac{2\pi k}{M}).
\]
Putting
\[
c_k(\Phi_1) = \cos(\Phi_1 - \tfrac{2\pi(k+1)}{M}) - \cos(\Phi_1 - \tfrac{2\pi k}{M})
\]
we can write
\begin{align*}
\sin&\fracpart{\lambda + \Phi_1} - \sin\fracpart{\Phi_1} \\
...
\end{align*}
where, for convenience we put $\eta_k(\lambda, \Phi_1) = c_k(\Phi_1) + \zeta_{k+1}(\lambda)$.  Observe that both $s_k$ and $c_k$ are continuous functions and that $s_k(\psi_{k}) = -2\sin(\tfrac{\pi}{M})$ and $c_k(\psi_{k}) = 0$.  
...
\]
Since the intervals $[\psi_{k-1}, \psi_{k})$ partion the real line, that is $\reals = \cup_{k\in\ints}[\psi_{k-1}, \psi_{k})$ then, for $\Phi_1 \in \reals$ we can write
\[
\sin\fracpart{\lambda + \Phi_1} - \sin\fracpart{\Phi_1} = v(\lambda, \Phi_1) + \lambda\cos\fracpart{\Phi_1}
\]
where $v(\lambda, \Phi_1) = \sum_{k\in\ints} v_k(\lambda, \Phi_1)$ and where
\[
v_k(\lambda,\Phi_1) = \begin{cases}
v_k(\Phi_1) + \lambda\eta_k(\lambda, \Phi_1) &  \Phi_1 \in [ \psi_{k} - \lambda,\psi_{k} ) \\
...
\end{align*}
Define the function $W(\lambda) = \expect R_1 v(\lambda, \Phi_1)$ so that $k_2(\lambda) = \lambda h_2(0) + W(\lambda).$  Lemma~\ref{lem:expRvlamphi} shows that $W(\lambda) =  \lambda(H_1 + \gamma(\lambda))$ where 
\[
H_1 = - 2 \sin(\tfrac{\pi}{M}) \sum_{k = 0}^{M-1} g\big((2k+1)\tfrac{\pi}{M}\big),
\]
and where $\gamma(\lambda)$ is a function that goes to zero as $\lambda$ goes to zero.  Because $\hat{\lambda}_L$ converges almost surely to zero as $L \rightarrow\infty$, then $\gamma(\hat{\lambda}_L)$ converges almost surely (and therefore also in probability) to zero as $L \rightarrow\infty$, i.e. $\gamma(\hat{\lambda}_L) = o_P(1)$ and therefore 
\[
...
$W(\hat{\lambda}_L) =  \hat{\lambda}_L(H + o_P(1))$
\end{lemma}
\begin{IEEEproof}
Recalling that $f(r,\phi)$ is the joint probability density function of $R_1$ and $\Phi_1$,
\begin{align*}
