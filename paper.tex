...
\]
Put $\lambda_L = \sfracpart{\theta_0 - \hat{\theta}}_\pi$ and $m_L = \hat{\rho} - \rho_0 G(0)$. If the periodic function $g\fracpart{\phi}_{\pi}$ is continuous at multiples of $\tfrac{\pi}{M}$, then the distribution of $(\sqrt{L}\lambda_L, \sqrt{L}m_L)$ converges to the bivariate normal with zero mean and covariance
\[
C = ?
\]
...
\[
\sqrt{L} Q(\hat{\lambda}_L) = \sqrt{L} \hat{\lambda}_L\big( ?  + o_P(1) \big)
\]
...
It follows that
\[
0 = o_P(1) + \sqrt{L}G_L^\prime(0) + \sqrt{L} \hat{\lambda}_L \big( ?  + o_P(1) \big)
\]
and rearranging gives,
\[
\sqrt{L} \hat{\lambda}_L = o_P(1) + \frac{\sqrt{L}G_L'(0)}{?  + o_P(1)}.
\]
...
\[
\sqrt{L} Q(\hat{\lambda}_L) = \sqrt{L} \hat{\lambda}_L\big( p + ?  + o_P(1) \big)
\]
\end{lemma}
\begin{IEEEproof}
Write
...
\begin{align*}
k_2(\lambda) &= \int_0^{2\pi}\int_{0}^\infty r \big( \sin\fracpart{\lambda + \phi} - \sin\fracpart{\phi} \big) f(r,\phi) dr d\phi \\
&= \int_0^{2\pi} \big( \sin\fracpart{\lambda + \phi} - \sin\fracpart{\phi} \big) g(\phi) d\phi.
\end{align*}
...
\end{align*}
when $\phi \in [\tfrac{ (2k-1)\pi}{M}, \tfrac{2k\pi}{M} - \lambda )$ and 
\begin{align*}
...
\end{align*}
when $\phi \in [ \tfrac{2k\pi}{M} - \lambda,\tfrac{2k\pi}{M} )$, and where
\[
...
The distribution of $\sqrt{L}G_L'(0)$ converges to the normal with zero mean and variance
\[
p \prob R_1^2 \sin^2(\Phi_1) + d \prob R_1^2 \sin^2\fracpart{\Phi_1}.
\]
\end{lemma}
