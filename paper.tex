...
\]
As is customary, let $(\Omega, \mathcal F, \prob)$ be the probability space over which the random variables $\{w_i\}$ are defined.  Let $A$ be the subset of the sample space $\Omega$ upon which $\sabs{G(\hat{\lambda}_L)} \rightarrow \sabs{G(0)}$ as $L\rightarrow\infty$.  Lemma~\ref{lem:convtoexpGlamL} shows that $\prob\{A\} = 1$.  Let $A'$ be the subset of the sample space upon which $\hat{\lambda}_L \rightarrow 0$ as $L\rightarrow \infty$.  Because $\sabs{G(x)}$ is uniquely maximised at $x=0$, it follows that $\sabs{G(\hat{\lambda}_L)} \rightarrow \sabs{G(0)}$ only if $\hat{\lambda}_L \rightarrow 0$ as $L \rightarrow\infty$. So $A \subseteq A'$ and therefore $\prob\{A'\} \geq \prob\{A\} = 1$.  Part one of Theorem~\ref{thm:consistency} follows.  
...
\[
0 \leq \sabs{G(\hat{\lambda}_L)} - \sabs{G(0)},
\]
and since $\hat{\lambda}_L$ is the maximiser of $\abs{G_L(x)}$,
\[ 
0 \leq \sabs{G_L(0)} - \sabs{G_L(\hat{\lambda}_L)}.
\]
Thus,
\begin{align*}
0 &\leq \sabs{G(\hat{\lambda}_L)} - \sabs{G(0)} \\ 
& \leq \sabs{G(\hat{\lambda}_L)} - \sabs{G(0)} + \sabs{G_L(0)} - \sabs{G_L(\hat{\lambda}_L)} \\
& \leq |G(\hat{\lambda}_L) - G_L(\hat{\lambda}_L)| + |G_L(0) - G(0)| \\
&\leq 2\sup_{\lambda \in [-\pi, \pi)} \sabs{G(\lambda) - G_L(\lambda)},
\end{align*}
...
\end{lemma}
\begin{IEEEproof}
Put $T(\lambda) = \expect G_L(\lambda)$ and write
\begin{align*}
&\sup_{\lambda \in [-\pi, \pi)}\sabs{G_L(\lambda) - G(\lambda)} \\
&\leq \sup_{\lambda \in [-\pi, \pi)}\sabs{G_L(\lambda) - T(\lambda) + T(\lambda) - G(\lambda)} \\
&\leq \sup_{\lambda \in [-\pi, \pi)}\sabs{G_L(\lambda) - T(\lambda)} +  \sup_{\lambda \in [-\pi, \pi)}\sabs{T(\lambda) - G(\lambda)}.
\end{align*}
Now,
\begin{align*}
T(\lambda) - G(\lambda) &= \big(\tfrac{\abs{P}}{L} - p\big)h_1(\lambda) + \big(\tfrac{\abs{D}}{L} - d\big)h_2(\lambda) \\
&= o(1) h_1(\lambda) + o(1) h_2(\lambda)
\end{align*}
where $o(1)$ denotes a number converging to zero as $L \rightarrow \infty$.  Since $\sabs{h_1(\lambda)} \leq 1$ and $\sabs{h_2(\lambda)} \leq 1$ for all $\lambda \in [-\pi, \pi)$, it follows that 
\[
\sup_{\lambda \in [-\pi, \pi)}\sabs{T(\lambda) - G(\lambda)} \rightarrow 0
\]  
almost surely as $L\rightarrow \infty$.  Lemma~\ref{lem:uniflawTGL} shows that 
\[
\sup_{\lambda \in [-\pi, \pi)}\sabs{G_L(\lambda) - T(\lambda)} \rightarrow 0
\]
almost surely as $L\rightarrow \infty$.
\end{IEEEproof}
...
Put $T(\lambda) = \expect G_L(\lambda)$, then
\[
\sup_{\lambda \in [-\pi, \pi)}\sabs{G_L(\lambda) - T(\lambda)} \rightarrow 0
\] 
almost surely as $L \rightarrow \infty$.
\end{lemma}
\begin{IEEEproof}
Put $D_L(\lambda) = G_L(\lambda) - T(\lambda)$ and let
\[
...
where 
\[
A_L = \sup_{n = 1,\dots,N}\sabs{D_L(\lambda_n)},
\]
\[
B_L = \sup_{n = 1,\dots,N}\sup_{\lambda \in L_n}\sabs{D_L(\lambda) - D_L(\lambda_n)}.
\]
Lemma~\ref{lem:supDLlambdan} shows that for any finite $N$, and any $\epsilon > 0$, 
\[
\prob\left\{ \lim_{L\rightarrow\infty} A_L > \epsilon \right\} = 0,
\]
i.e., $A_L \rightarrow 0$ almost surely as $L\rightarrow\infty$.  Lemma~\ref{lem:supsupDLlambda} shows that for any $\epsilon > 0$,
\[
\prob\left\{ \lim_{L \rightarrow \infty} B_L > \epsilon + \tfrac{4\pi }{N}\expect R_i \right\} = 0.
\] 
...
&\leq \prob\left\{\lim_{L\rightarrow\infty}(A_L + B_L) > 3\epsilon \right\} \\
&\leq \prob\left\{\lim_{L\rightarrow\infty} (A_L + B_L) > \epsilon + \epsilon + \tfrac{4\pi }{N}\expect R_i \right\} \\
&\leq \prob\left\{\lim_{L\rightarrow\infty} A_L > \epsilon\right\} +  \prob\left\{ \lim_{L\rightarrow\infty} B_L > \epsilon + \tfrac{4\pi }{N}\expect R_i \right\} \\
&= 0.
\end{align*}
...
If $N$ is finite then $A_L \rightarrow 0$ almost surely as $L \rightarrow \infty$.
\end{lemma}
\begin{IEEEproof}
Put
\[
...
so that
\[
D_L(\lambda) = G_L(\lambda) - T(\lambda) = \frac{1}{L}\sum_{i=1}^{L} \big( Z_i(\lambda) - \expect Z_i(\lambda) \big).
\]
...
\[
A_L = \sup_{n=1,\dots,N}\sabs{D_L(\lambda_n)} \leq \sum_{n=1}^N \sabs{D_L(\lambda_n)} \rightarrow 0
\]
...
\[
\prob\left\{ \lim_{L \rightarrow \infty} B_L > \epsilon + \tfrac{2\pi}{N}\expect R_i \right\} = 0.
\]
\end{lemma}
\begin{IEEEproof}
Observe that
...
&\leq  \abs{G_L(\lambda) - \expect G_L(\lambda) - G_L(\lambda_n) + \expect G_L(\lambda_n)} \\
...
then
\[
B_L = \sup_{n = 1,\dots,N}\sup_{\lambda \in L_n}\sabs{D_L(\lambda) - D_L(\lambda_n)} \leq C_L + \expect C_L.
\]
because 
\[
 \sup_{n = 1,\dots,N}\sup_{\lambda \in L_n} \expect \abs{G_L(\lambda) - G_L(\lambda_n)} \leq \expect C_L
\]
follows by moving expectation outside the supremum.  Lemma~\ref{lem:CL} shows that $\expect C_L \leq \tfrac{2\pi}{N}\expect R_i$ and also that
\[
\prob\left\{ \lim_{L \rightarrow \infty} C_L > \epsilon + \tfrac{2\pi}{N}\expect R_i \right\} = 0.
\]
So,
\begin{align*}
\prob&\left\{ \lim_{L \rightarrow \infty} B_L > \epsilon + \tfrac{4\pi}{N}\expect R_i \right\} \\
&\leq \prob\left\{ \lim_{L \rightarrow \infty} (C_L + \expect C_L) > \epsilon + \tfrac{4\pi}{N}\expect R_i \right\} \\
&\leq \prob\left\{ \lim_{L \rightarrow \infty} C_L > \epsilon + \tfrac{2\pi}{N}\expect R_i \right\} = 0.
\end{align*}
\end{IEEEproof}

\begin{lemma}\label{lem:CL}
$\expect C_L \leq \tfrac{2\pi}{N}\expect R_i$ and for any $\epsilon > 0$,
\[
\prob\left\{ \lim_{L \rightarrow \infty} C_L > \epsilon + \tfrac{2\pi}{N}\expect R_i \right\} = 0.
\]
\end{lemma}
...
\[
\abs{\cos(\lambda) - \cos(\lambda_n)} \leq \tfrac{2\pi}{N} \;\;\; \text{and} \;\;\; \abs{\cos\sfracpart{\lambda} - \cos\sfracpart{\lambda_n}} \leq \tfrac{2\pi}{N}.
\]
Because this results does not depend on $n$,
\[
\sup_{n=1,\dots,N}\sup_{\lambda \in L_n} \abs{Z_i(\lambda) - Z_i(\lambda)} \leq R_i \frac{2\pi}{N}
\]
for all $i = 1, \dots, L$.  Now,
\begin{align*}
...
&\leq \frac{2\pi}{N L}\sum_{i=1}^L R_i \rightarrow \frac{2\pi}{N}\expect R_i
\end{align*}
almost surely to $\frac{2\pi}{N}\expect R_i$ as $L \rightarrow\infty$ by the strong law of large numbers, and so
\begin{align*}
\prob&\left\{ \lim_{L \rightarrow \infty} C_L > \epsilon + \tfrac{2\pi}{N}\expect R_i \right\} \\
&\leq \prob\left\{ \lim_{L \rightarrow \infty} \frac{2\pi}{N L}\sum_{i=1}^L R_i > \epsilon + \tfrac{2\pi}{N}\expect R_i \right\} = 0.
\end{align*}
Also $\expect C_L \leq \frac{2\pi}{N}\expect R_i$.
\end{IEEEproof}
...
\[
\abs{\cos(x + \delta) - \cos(x)} \leq \abs{\delta},
\]
and 
\[
