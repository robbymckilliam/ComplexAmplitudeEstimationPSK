...
\]
Put $\lambda_L = \sfracpart{\theta_0 - \hat{\theta}}_\pi$ and $m_L = \hat{\rho} - \rho_0 G(0)$. If the periodic function $g\fracpart{\phi}_{\pi}$ is continuous at $\phi = (2k+1)\tfrac{\pi}{M}$ for every integer $k$, then the distribution of $(\sqrt{L}\lambda_L, \sqrt{L}m_L)$ converges to the bivariate normal with zero mean and covariance
\[
...
\]
and put $\hat{\lambda}_L = \langle\theta_0 - \hat{\theta}\rangle$.  Then $\hat{\lambda}_L$ is the maximiser of $G_L(\lambda)^2$ and equivalently the maximiser of $\abs{G_L(\lambda)}$.  We will show that $\hat{\lambda}_L$ converges almost surely to zero as $L \rightarrow \infty$.  From this, the proof of strong consistency follows.
...
\]
and since $v_k(\lambda,\phi) = 0$ for $\phi \in [0,2\pi)$ when $k \notin \{0,1,\dots,M\}$ the infinite sum above can be truncated so that
\[
k_2(\lambda) = \sum_{k = 0}^M \int_0^{2\pi} v_k(\lambda, \phi) g(\phi) d\phi
\]
Now
\begin{align*}
...
\end{align*}
Now, since $g\fracpart{\phi}_\pi$ is continuous at $(2k+1)\tfrac{\pi}{M}$ for each integer $k$, and $v_k(\phi)$ is continuous and $v_k(\tfrac{(2k+1)\pi}{M}) =  2\sin(\tfrac{\pi}{M})$,
\begin{align*}
...
and
\begin{align*}
k_2(\lambda) &= \sum_{k = 0}^M \lambda( 2\sin(\tfrac{\pi}{M})g(\tfrac{(2k+1)\pi}{M}) + \nu_k(\lambda) + \zeta_k(\lambda) A_k ) \\
...
\[
k_2(\hat{\lambda}_L) = o_P(1) + 2 \hat{\lambda}_L \sin(\tfrac{\pi}{M}) \sum_{k = 0}^M g(\tfrac{(2k+1)\pi}{M})
\]
as required.
\end{IEEEproof}
