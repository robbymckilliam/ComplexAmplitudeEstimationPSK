...
% use italics version of capital Greek letters
\renewcommand{\Phi}{\varPhi}
\renewcommand{\Theta}{\varTheta}

%opening
\title{Asymptotic statistics of carrier phase and amplitude estimation for PSK using pilots and data}
\author{Robby McKilliam, Andr\'{e} Pollok, Bill Cowley, I. Vaughan L. Clarkson and Barry Quinn  
\thanks{
A preliminary version of this paper was presented at ICASSP'13~\cite{McKilliam_leastsqPSKnoncoICASSP_2012}.  Supported under the Australian Government's Australian Space Research Program.
...
\comment{
Doesn't Bill's paper \cite{Cowley_ref_sym_carr_1998} consider the case with pilots?
}

\comment{
The existing literature~\cite{Mackenthun1994,Cowley_ref_sym_carr_1998,ViterbiViterbi_phase_est_1983,Sweldens2001,Wilson1989,Makrakis1990,Liu1991} mostly considers \emph{non-data aided} estimation, where no pilot symbols exist ($P = \emptyset$). In this setting \emph{differential encoding} and \emph{differential (i.e. noncoherent) detection} are often used to handle the resulting $M$-fold ambiguity of the phase estimate.  A popular non-data aided estimator was proposed by Viterbi and Viterbi~\cite{ViterbiViterbi_phase_est_1983}.%
}
The idea is to `strip' the modulation from the received signal by taking $y_i / \abs{y_i}$ to the power of $M$.  A function $F: \reals \mapsto \reals$ is chosen and the estimator of the carrier phase $\theta_0$ is taken to be $\tfrac{1}{M}\angle{C}$ where $\angle$ denotes the complex argument and
...
An important paper is by Mackenthun~\cite{Mackenthun1994} who described an algorithm to compute the least squares estimator requiring only $O(L \log L)$ arithmetic operations.  Sweldens~\cite{Sweldens2001} rediscovered Mackenthun's algorithm in 2001.  Both Mackenthun and Sweldens considered only the non-data aided setting, but we show in Section~\ref{sec:least-squar-estim}~that Mackenthun's algorithm can be modified to include pilot symbols. Our model includes the non-data aided case by setting the number of pilot symbols to zero, that is, putting $P = \emptyset$.  

In the literature it has been common to assume that the data symbols $\{d_i, i \in D\}$ are of primary interest and that the complex amplitude $a_0$ is a nuisance parameter.  The metric of performance is correspondingly the \emph{symbol error rate}, or \emph{bit error rate}.  While estimating the data symbols is ultimately the goal, we take the opposite point of view here.  Our aim is to estimate $a_0$, and we treat the unknown data symbols as nuisance parameters.  This is motivated by the fact that in many modern communication systems the data symbols are \emph{coded}.  For this reason raw symbol error rate is not of interest at this stage.  Instead, we desire an accurate estimator $\hat{a}$ of $a_0$, so that the compensated received symbols $\hat{a}^{-1}y_i$ can be accurately modelled using an additive noise channel.  The additive noise channel is a common assumption for subsequent receiver operations, such as decoding.  The estimator $\hat{a}$ is also used in the computation of decoder metrics for modern decoders, and for interference cancellation in multiuser systems.  \comment{Furthermore, iterative receiver synchronisation methods that are aided by the error correcting code, also known as \emph{turbo synchronisation} \cite{Lottici2004,Noels2005}, %\cite{Herzet2007,Herzet2007_TurboSyncFramework}
typically converge correctly only if initialised at some $a$ sufficiently close to $a_0$. Obtaining an accurate estimator $\hat{a}$ of $a_0$ is therefore a crucial ingredient for an effective turbo synchronisation receiver.} Consequently, our primary metric of performance will not be symbol or bit error rate, but the estimation error.  \comment{However, we will also present packet error rate results for an LDPC-coded satellite communication system to demonstrate the use of the proposed estimator in a practical system\dots}
%It will be informative to consider the carrier phase and amplitude estimators separately, that is, if $\hat{a} = \hat{\rho}e^{j\hat{\theta}}$ where $\hat{\rho}$ is a positive real number, then we consider $\sabs{\langle\theta_0 - \hat{\theta}\rangle_\pi}$ and $\sabs{\hat{\rho} - \rho_0}$.  The function $\fracpart{\cdot}_\pi$ denotes its argument taken `modulo $2\pi$' into the interval $[-\pi, \pi)$.  It will become apparent why $\langle\theta_0 - \hat{\theta}\rangle_\pi$ rather than $\theta_0 - \hat{\theta}$ is the appropriate measure of error for the phase parameter.  It is usual to write \emph{estimator} minus \emph{truth}, i.e., $\hat{\theta} - \theta_0$ rather than $\theta_0 - \hat{\theta}$.  However, it is notationally convenient to use $\langle\theta_0 - \hat{\theta}\rangle_\pi$ rather than $\langle\hat{\theta} - \theta_0\rangle_\pi$ here.
...
%In Section~\ref{sec:gaussian-noise-case} we consider the special case when the noise is Gaussian.  In this case, our expressions for the asymptotic distribution can be simplified.  
In Section~\ref{sec:simulations} we present the results of Monte-Carlo simulations.  These simulations agree with the derived asymptotic properties. 
...
%In this section we derive Mackentun's algorithm to compute the least squares estimator of the carrier phase and amplitude~\cite{Mackenthun1994}.  
Mackenthun specifically considered the non-data aided setting and assumed differential encoding and detection to deal with the $M$-fold phase ambiguity \cite{Mackenthun1994}. In contrast, we consider non-differential encoding with coherent detection and to this end, extend Mackenthun's algorithm to include pilot symbols.

For the purpose of analysing computational complexity, we will assume that the number of data symbols $\abs{D}$ is proportional to the total number of symbols $L$, so that, for example, $O(L) = O(\abs{D})$.  In this case Mackentun's algorithm requires $O(L \log L)$ arithmetic operations.  This complexity arises from the need to sort a list of $\abs{D}$ elements.  
...
%In Section~\ref{sec:line-time-algor} we will show that a full sort is not necessary, and that the least squares estimator can be implemented in $O(L)$ operations.

Our starting point is the sum of squares function defined in \eqref{eq:SSdefn}, which can be rewritten as
%Define the sum of squares function
\begin{align}
SS(a, &\{d_i, i \in D\}) = \sum_{i \in P \cup D} \abs{ y_i - a s_i }^2 \nonumber \\
&= \sum_{i \in P \cup D} \big( \abs{y_i}^2 - a s_i y_i^* - a^* s_i^* y_i + aa^* \big), \label{eq:SS}
\end{align}
where $*$ denotes complex conjugation.  The minimiser of $SS$ with respect to $a$ as a function of $\{d_i, i \in D\}$ is
\begin{equation}\label{eq:hata}
\hat{a}(\{d_i, i \in D\}) = \frac{1}{L} \sum_{i \in P \cup D} y_i s_i^* = \frac{1}{L} Y(\{d_i, i \in D\}),
\end{equation}
where $L = \abs{P \cup D}$ and to simplify our notation we have put 
\begin{align}\label{eq:Y}
	Y(\{d_i, i \in D\}) = \sum_{i \in P \cup D} y_i s_i^* 
		%= \sum_{i \in P } y_i p_i^* + \sum_{i \in D } y_i d_i^*
		= B + \sum_{i \in D } y_i d_i^*,
\end{align}
where $B = \sum_{i \in P} y_i p_i^*$ is independent of the data symbols.  For notational brevity, we also write $Y$ instead of $Y(\{ d_i, i \in D\})$.  Substituting $\hat{a}$ from \eqref{eq:hata} for $a$ into~\eqref{eq:SS} we obtain $SS$ minimised with respect to $a$,
\begin{equation}\label{eq:SSdatasymbols}
	SS(\{d_i, i \in D\}) = A - \frac{1}{L}\abs{Y(\{d_i, i \in D\})}^2,
\end{equation}
where $A = \sum_{i \in P \cup D}\abs{y_i}^2$ does not depend on the $d_i$.  The least squares estimators of the data symbols, denoted $\{\hat{d}_i,i\in D\}$, are the minimisers of~\eqref{eq:SSdatasymbols} or, equivalently, the maximisers of $\abs{Y}^2$.
%\begin{align}
%	Q(\{d_i, i \in D\}) = \abs{Y(\{d_i, i \in D\})}^2.
%\end{align}
Observe that given candidate values for the data symbols, we can compute the corresponding $\abs{Y}^2$ in $O(L)$ arithmetic operations.  The na\"{\i}ve approach is to compute the objective function $\abs{Y}^2$ for all possible $M^\abs{D}$ data sequences and select the sequence that yields the maximum value.  Using Mackenthun's algorithm, we are able to avoid this computationally expensive exhaustive search, which would require $O(LM^L)$ arithmetic operations.

First consider the case without pilot symbols, i.e.\ $P=\emptyset$. In this case, $\abs{Y}^2$ is identical to the objective function considered by Mackenthun \cite[Eq.\ (2)]{Mackenthun1994}.  This equivalence stems from the fact that the least squares estimators $\hat{d}_i$ are also the maximum likelihood estimators under the additive white Gaussian noise assumption of \cite{Mackenthun1994}.  Consequently, Mackenthun's algorithm as presented in~\cite{Mackenthun1994} or~\cite{Sweldens2001} can be applied without modification to obtain the $\hat{d}_i$.  As shown in~\cite{Sweldens2001,Mackenthun1994}, at most $M\abs{D}$ candidate sequences need to be trialled. In fact, due to an $M$-fold phase ambiguity in the absence of pilot symbols, the objective function $\abs{Y}^2$ is periodic in $\abs{D}$ and thus, only $\abs{D}$ of the $M\abs{D}$ candidate sequences need to be considered. We do not explicitly state the algorithm at this point, but instead describe it below for the more general setting with pilots.

In the presence of pilot symbols, all $M\abs{D}$ candidate sequences are relevant and we rely on the pilots to resolve the phase ambiguity. In the following we describe how these sequences can be enumerated. Let $\hat{b}_i$ denote the hard decision on the received data symbol $y_i$, $i \in D$, i.e.
\[
	\hat{b}_i = e^{j\hat{u}_i}, \qquad \text{where} \qquad \hat{u}_i = \round{\angle y_i},
\]
where $\angle(\cdot)$ denotes the complex argument (or phase), and $\round{\cdot}$ rounds its argument to the nearest multiple of $\frac{2\pi}{M}$.  A word of caution, the notation $\round{\cdot}$ is often used to denote rounding to the nearest \emph{integer}.  This is not the case here.  If the function $\operatorname{round}(\cdot)$ takes its argument to the nearest integer\footnote{The direction of rounding for half-integers is not important so long as it is consistent.  We have chosen to round up half-integers here.} then,
\[
\round{x} = \tfrac{2\pi}{M}\operatorname{round}\left(\tfrac{M}{2\pi}x\right).
\]

Let $S$ denote the set of all $M\abs{D}$ candidate sequences.  The first sequence in $S$ is 
\[
f_0 = \{ \hat{b}_i, i \in D \}.
\]  
Now let
\[
	z_i = \angle(y_i \hat{b}_i^*) = \angle{y_i} - \hat{u}_i = \angle{y_i} - \round{\angle{y_i}}
\]
and let $\sigma$ denote the permutation of the indices in $D$ such that $z_{\sigma(i)}$ are in ascending order, that is,
\begin{equation}\label{eq:sigmasortind}
	z_{\sigma(i)} \leq z_{\sigma(k)},
\end{equation}
whenever $i < k $ where $i, k \in \{0, 1, \dots, \abs{D}{-}1\}$.

The next sequence $f_1$ is given by replacing the element $b_{\sigma(0)}$ in $f_0$ with $b_{\sigma(0)}e^{-j2\pi/M}$.  Given a sequence $x$ we use $x e_i$ to denote $x$ with the $i$th element replaced by $x_i e^{-j2\pi/M}$.  Using this notation, $f_1 = f_0 e_{\sigma(0)}$ and more generally, the $k$th sequence in $S$ is
\begin{equation}\label{eq:fkrec}
f_{k+1} = f_{k} e_{\sigma(k)}.
\end{equation}
In this way, all $M\abs{D}$ sequences in $S$ can be recursively enumerated.

Let $Y_k = Y( f_k )$.  We want to find the $f_k \in S$ corresponding to the maximiser $\{\hat{d}_i,i\in D\}$ of $\abs{Y_k}^2$.  A na\"{\i}ve approach would be to compute $\abs{Y_k}^2$ for each $k \in \{0,1,\dots,M\abs{D}{-}1\}$.  Computing $\abs{Y_k}^2$ for any particular $k$ requires $O(L)$ arithmetic operations.  So, this na\"{\i}ve approach would require $O(L M \abs{D}) = O(L^2)$ operations in total.  Following Mackenthun~\cite{Mackenthun1994}, we show how to recursively compute $\abs{Y_k}^2$.

Using \eqref{eq:Y}, $Y_k$ can be rewritten as
\begin{align*}
	Y_k = Y( f_k ) 
		&= B  + \sum_{i \in D} y_i f_{ki}^*
		= B + \sum_{i \in D}g_{ki},
\end{align*}
where $f_{ki}$ denotes the $i$th symbol in $f_k$, and for convenience, we put $g_{ki}  = y_i f_{ki}^*$.  Letting $g_{k}$ be the sequence $\{g_{ik}, i \in D\}$ we have, from~\eqref{eq:fkrec}, that $g_k$ satisfies the recursive equation
\[
...
\]
So, each $Y_k$ can be computed from its predecessor $Y_{k-1}$ in $O(1)$ arithmetic operations.  
%Given $Y_k$, the value of
%\begin{equation*}
%	SS(f_k) = A - \frac{1}{L}\abs{Y(f_k)}^2
%		= A - \frac{1}{L}\abs{Y_k}^2
%\end{equation*}
%can be computed in $O(1)$ operations.  Let $\hat{k} = \arg\min_k SS(f_k)$.  
Letting $\hat{k} = \arg\max_k \abs{Y_k}^2$, the least squares estimator of $a_0$ is then computed according to~\eqref{eq:hata},
\begin{equation}\label{eq:ahatYhat}
	\hat{a} = \frac{1}{L} Y_{\hat{k}}.
\end{equation}
Pseudocode is given in Algorithm~\ref{alg:loglinear}.  Line~\ref{alg_sortindices} contains the function $\operatorname{sortindices}$ that, given $z = \{z_i, i \in D\}$, returns the permutation $\sigma$ as described in~\eqref{eq:sigmasortind}.  The $\operatorname{sortindices}$ function requires sorting $\abs{D}$ elements.  This requires $O(L \log L)$ operations.  The $\operatorname{sortindices}$ function is the primary bottleneck in this algorithm when $L$ is large.  The loops on lines~\ref{alg_loop_setup} and~\ref{alg_loop_search} and the operations on lines~\ref{alg_Y} to lines~\ref{alg_Q} all require $O(L)$ or less operations.  %In the next sections we will show how the sortindices function can be avoided.  This leads to an algorithm that requires only $O(L)$ operations.

Recalling that $f_{\hat{k}}$ is the sequence of least squares estimators $\{\hat{d}_i, i\in D\}$, we can rewrite $\hat{a}$ in \eqref{eq:ahatYhat} as
\begin{align*}
	\hat{a} = \frac{1}{L} Y_{\hat{k}}
		= \frac{1}{L} Y(\{\hat{d}_i,i\in D\})
		= \frac{1}{L} \sum_{i \in P} y_i p_i^*  + \frac{1}{L} \sum_{i \in D} y_i \hat{d}_{i}^*
\end{align*}
to illustrates that the first and second terms of the estimator operate in \emph{data-aided} and \emph{decision-directed} mode, respectively. If no pilots are present, $\abs{P}=0$ and the estimator is purely decision-directed. Conversely, not utilising any data symbols for estimation purposes, i.e.\ $\abs{D}=0$, corresponds to a fully data-aided estimator.
...
%\]
%be a function mapping the interval $[0, 2\pi)$ to a sequence of $M$-PSK symbols indexed by the elements of $D$.  Observe that $f(\theta)$ is piecewise continuous \comment{$\leftarrow$ constant?}.  The subintervals of $[0, 2\pi)$ over which $f(\theta)$ remains constant are determined by the values of $\{z_i, i \in D\}$.  Let
%\[
...
%Y_{k+1} = Y_k + \eta g_{k\sigma(k)}.
%\]
%So, each $Y_k$ can be computed from its predecessor $Y_{k-1}$ in $O(1)$ arithmetic operations.  Given $Y_k$, the value of $SS(f_k)$ can be computed in $O(1)$ operations using~\eqref{eq:SSfk}.  Let $\hat{k} = \arg\min_k SS(f_k)$.  The least squares estimator of $a_0$ is then computed according to~\eqref{eq:hata},
%\begin{equation}\label{eq:ahatYhat}
%\hat{a} = \frac{1}{L} Y_{\hat{k}}.
%\end{equation}
%Pseudocode is given in Algorithm~\ref{alg:loglinear}.  Line~\ref{alg_sortindices} contains the function $\operatorname{sortindices}$ that, given $z = \{z_i, i \in D\}$, returns the permutation $\sigma$ as described in~\eqref{eq:sigmasortind}.  The $\operatorname{sortindices}$ function requires sorting $\abs{D}$ elements.  This requires $O(L \log L)$ operations.  The $\operatorname{sortindices}$ function is the primary bottleneck in this algorithm when $L$ is large.  The loops on lines~\ref{alg_loop_setup} and~\ref{alg_loop_search} and the operations on lines~\ref{alg_Y} to lines~\ref{alg_Q} all require $O(L)$ or less operations.  %In the next sections we will show how the sortindices function can be avoided.  This leads to an algorithm that requires only $O(L)$ operations.
...
In this section we describe the asymptotic properties of the least squares estimator.  \comment{It will be informative to consider the carrier phase and amplitude estimators separately, that is, if $\hat{a} = \hat{\rho}e^{j\hat{\theta}}$, where $\hat{\rho}$ is a positive real number, then we consider the estimation errors $\sabs{\langle\theta_0 - \hat{\theta}\rangle_\pi}$ and $\sabs{\hat{\rho} - \rho_0}$.  The function $\sfracpart{x}_\pi$ denotes $x$ taken `modulo $2\pi$' into the interval $[-\pi, \pi)$, that is
\[
\fracpart{x}_\pi = x - 2\pi\operatorname{round}\left(\frac{x}{2\pi}\right).
\]
In what follows we similarly use $\sfracpart{x}$ to denote $x$ taken `modulo $\tfrac{2\pi}{M}$' into the interval $\left[-\tfrac{\pi}{M}, \tfrac{\pi}{M}\right)$, that is
\[
...
\comment{
It will become apparent why $\langle\theta_0 - \hat{\theta}\rangle_\pi$ rather than $\theta_0 - \hat{\theta}$ is the appropriate measure of error for the phase parameter.  It is usual to write \emph{estimator} minus \emph{truth}, i.e., $\hat{\theta} - \theta_0$ rather than $\theta_0 - \hat{\theta}$.  However, it is notationally convenient to use $\langle\theta_0 - \hat{\theta}\rangle_\pi$ rather than $\langle\hat{\theta} - \theta_0\rangle_\pi$ here.
}
...
\] 
Let $R_i \geq 0$ and $\Phi_i \in [0,2\pi)$ be real random variables satisfying (see Appendix~\ref{apdx:circ-symm-compl})
...
\label{thm:consistency:part1}
\item $\hat{\rho} \rightarrow \rho_0 G(0)$ almost surely as $L \rightarrow \infty$.
\label{thm:consistency:part2}
\end{enumerate}
\end{theorem}

\comment{CAUTION: Theorem~\ref{thm:consistency} does not include the case without pilots, i.e.\ $p=0$, which was discussed in our ICASSP paper \cite{McKilliam_leastsqPSKnoncoICASSP_2012}. In this case, $G(x)$ is uniquely maximised at $x = 0$ over the interval $[-\frac{\pi}{M},\frac{\pi}{M})$ such that $\sfracpart{\theta_0 - \hat{\theta}}_\pi$ in part 1 of the theorem needs to be replaced by $\sfracpart{\theta_0 - \hat{\theta}}$.}

\begin{theorem}\label{thm:normality} (Asymptotic normality)
Under the same conditions as Theorem~\ref{thm:consistency}, let $f(r,\phi)$ be the joint probability density function of $R_1$ and $\Phi_1$ (see Appendix~\ref{apdx:circ-symm-compl}), let
\[
...
\]
\begin{alignat*}{5}
A_1 &= \expect R_1^2\sin^2(\Phi_1), &A_2 &= \expect R_1^2\sin^2\fracpart{\Phi_1},
\\
B_1 &= \expect R_1^2 \cos^2(\Phi_1) - 1,\qquad  &B_2 &= \expect R_1^2 \cos^2\fracpart{\Phi_1} - h_2^2(0).
\end{alignat*}
\end{theorem}

\comment{
Part~\ref{thm:consistency:part1} of Theorem~\ref{thm:consistency} shows that the phase estimation error $\langle\theta_0 - \hat{\theta}\rangle_\pi$ converges almost surely to zero as $L\rightarrow \infty$, i.e.\ the estimator $\hat{\theta}$ is strongly consistent. In contrast, part~\ref{thm:consistency:part2} of the theorem shows that $\hat{\rho}$ is not a consistent estimator of the true amplitude $\rho_0$ and converges to $\rho_0 G(0)$.  The value $G(0)$ is a function of the noise distribution and $G(0)$ is close to $1$ when the variance of the noise is small, but grows without bound as the noise variance increases.  That is, the asymptotic bias is small when the SNR is large, but significant when the SNR is small.  We will further investigate the bias in Sec.~\ref{sec:simulations}.  Theorem~\ref{thm:normality} shows that $\sqrt{L}(\hat{\rho} - \rho_0 G(0))$ (i.e.\ the amplitude estimation error after bias removal) and $\sqrt{L}\langle\theta_0 - \hat{\theta}\rangle_\pi$ are asymptotically Gaussian distributed and statistically independent as $L\rightarrow\infty$.
}

\comment{
We would like to stress that many practical estimators such as the SNR estimators in~\cite{Pauluzzi2000} are biased and particularly at low SNR, the bias can be large.  To the best of the authors' knowledge, the value of the bias of these estimators has only been determined by computer simulation and no theoretical analysis exists.  In contrast, we have presented a closed-form expression for the asymptotic bias of $\hat\rho$, thereby laying the foundation for an improved estimator.  However, removal of the bias requires knowledge of $G(0)$, which may not be available at the receiver. Estimation of $G(0)$ is beyond the scope of this paper.
}
...
Theorem~\ref{thm:normality} requires the function $g$ to be continuous at $\tfrac{2\pi}{M}k + \tfrac{\pi}{M}$ for each $k = 0, \dots, M{-}1$.  This places mild restrictions on the distribution of the noise $w_i$.  For example, the requirements are satisfied if the joint probability density function (pdf) of the real and imaginary parts of $w_i$ is continuous, since in this case $f(r,\phi)$ is continuous.  Because $f(r,\phi)$ has period $2\pi$ and is even on $[-\pi,\pi)$ with respect to $\phi$ (see Appendix~\ref{apdx:circ-symm-compl}) it follows that $g$ has period $2\pi$ and is even on $[-\pi, \pi)$.
...
\comment{
Let $a = \rho e^{j\theta}$.  Slightly abusing notation by reusing $SS$, let $SS(\rho, \theta, \{d_i, i \in D\}) = SS(\rho e^{j\theta}, \{d_i, i \in D\})$. From \eqref{eq:SS},
\begin{align}
	SS(\rho, \theta, &\{d_i, i \in D\}) = \sum_{i \in P \cup D} \abs{ y_i - \rho e^{j\theta} s_i }^2  \nonumber \\
	&= \sum_{i \in P} \abs{ y_i - \rho e^{j\theta} p_i }^2 + \sum_{i \in D} \abs{ y_i - \rho e^{j\theta} d_i }^2. \label{eq:SSallparams}
\end{align}
%This should not cause confusion as $SS(a, \{d_i, i \in D\})$, $SS(\rho, \theta, \{d_i, i \in D\})$, and $SS(\{d_i, i \in D\})$ are easily told apart by their arguments.  
For given $\theta$, the least squares estimator of the $i$th data symbol $d_i$ minimises $\abs{ y_i - \rho e^{j\theta} d_i }^2$, that is,
\begin{equation}\label{eq:hatdfinxtheta}
\hat{d}_i(\theta) = e^{j\hat{u}_i(\theta)}, \qquad \text{where} \qquad \hat{u}_i(\theta) = \round{\angle( e^{-j\theta}y_i)}.
\end{equation}
As defined here, $\hat{u}_i(\theta)$ is not strictly inside the set $\{0, \tfrac{2\pi}{M}, \dots, \tfrac{2\pi(M-1)}{M}\}$, but this is not of consequence, as we intend its value to be considered equivalent modulo $2\pi$.  With this in mind,
\[
\hat{u}_i(\theta) = \round{\angle{y_i} - \theta },
\]
which is equivalent to the definition from~\eqref{eq:hatdfinxtheta} modulo $2\pi$ and hence, we only require to consider $\theta$ in the interval $[0, 2\pi)$.
}

\comment{
Note from \eqref{eq:hatdfinxtheta} that $\hat{d}_i(\theta)$ does not depend on $\rho$.  Thus, if $\hat{\theta}$ is the least squares estimator of the phase, then the sequence $\{ \hat{d}_i(\hat{\theta}), i \in D \}$ corresponds to the least squares estimators of the data symbols that minimise~\eqref{eq:SSdatasymbols}, i.e.\ $\{ \hat{d}_i=\hat{d}_i(\hat{\theta}), i \in D \}$.
}
...
Z(\theta)  = \sum_{i \in P} y_i e^{-j\theta} p_i^* + \sum_{i \in D} y_i e^{-j\theta} \hat{d}_i^*(\theta).
	\label{eq:Z}
\end{align}
Differentiating with respect to $\rho$ and setting the resulting expression to zero gives the least squares estimator of $\rho_0$ as a function of $\theta$,
...
By definition the amplitude $\rho_0$ and its estimator $\hat{\rho}$ are positive.  However, $\hat{\rho}(\theta) = \tfrac{1}{L}\Re(Z(\theta))$ may take negative values for some $\theta \in [-\pi,\pi)$.  %In this case we would replace $\theta$ with $\fracpart{\theta + \phi}$ and $\hat{\rho}(\theta)$ with $-\hat{\rho}(\theta)$ and observe that
%\[
%SS(\rho,\theta) = SS(-\rho, \fracpart{\theta + \pi})
%\]
The least squares estimator $\hat{\theta}$ of $\theta_0$ is the minimiser of $SS(\theta)$ under the constraint $\hat{\rho}(\theta) = \tfrac{1}{L}\Re(Z(\theta)) > 0$.  Equivalently $\hat{\theta}$ is the maximiser of $\Re(Z(\theta))$ with no constraints required.
...
\end{align*}
where we put $\lambda = \fracpart{\theta_0 - \theta}_\pi$ and where, as in before, we consider $\hat{u}_i(\theta)$ equivalent modulo $2\pi$.  Because $\hat{d}_i^*(\theta) = e^{-j\hat{u}_i(\theta)}$, it follows that, when $i \in D$,
\begin{align}
...
\]
Now, \eqref{eq:Z} can be rewritten as
\[
...
\]
As is customary, let $\varOmega$ be the sample space on which the random variables $\{w_i\}$ are defined.  Let $\varLambda$ be the subset of the sample space $\varOmega$ on which $G(\hat{\lambda}_L) \rightarrow G(0)$ as $L\rightarrow\infty$.  Lemma~\ref{lem:convtoexpGlamL} in Appendix~\ref{apdx:proof-almost-sure} shows that $\prob\{\varLambda\} = 1$.  Let $\varLambda'$ be the subset of the sample space on which $\hat{\lambda}_L \rightarrow 0$ as $L\rightarrow \infty$.  Because $G(x)$ is uniquely maximised at $x=0$, it follows that $G(\hat{\lambda}_L) \rightarrow G(0)$ only if $\hat{\lambda}_L \rightarrow 0$ as $L \rightarrow\infty$. So $\varLambda \subseteq \varLambda'$ and therefore $\prob\{\varLambda'\} \geq \prob\{\varLambda\} = 1$.  Part~\ref{thm:consistency:part1} of Theorem~\ref{thm:consistency} follows.  

It remains to prove part~\ref{thm:consistency:part2} of the theorem regarding the convergence of the amplitude estimator $\hat{\rho}$.  From~\eqref{eq:hatrhoZ} and \eqref{eq:GLdefn},
\begin{equation}\label{eq:rhoGLZ}
\hat{\rho} = \hat{\rho}(\hat{\theta}) = \frac{1}{L}\Re(Z(\hat{\theta})) = \rho_0 G_L(\hat{\lambda}_L).
\end{equation}  
Lemma~\ref{lem:GLtoG0} in Appendix~\ref{apdx:proof-almost-sure} shows that $G_L(\hat{\lambda}_L)$ converges almost surely to $G(0)$ as $L\rightarrow\infty$, and $\hat{\rho}$ consequently converges almost surely to $\rho_0 G(0)$ as required.  %It remains to prove Lemmas~\ref{lem:convtoexpGlamL} and~\ref{lem:GLtoG0}.  These are proved in %Section~\ref{sec:proof-almost-sureappendix} of 
%the appendix.

\section{Proof of asymptotic normality (Theorem~\ref{thm:normality}) } \label{sec:proof-asympt-norm}

The proof of Theorem~\ref{thm:normality} hinges on Lemmas \ref{lem:diffatlambdaL}--\ref{lem:XL}, which can be found in Appendix~\ref{apex:proof-asymptotic-normality}.  We first prove the asymptotic normality of $\sqrt{L} \hat{\lambda}_L$.  Once this is done we will be able to prove the normality of $\sqrt{L} \hat{m}_L$.  Recall that $\hat{\lambda}_L$ is the maximiser of the function $G_L$ defined in~\eqref{eq:GLdefn}.  The proof is complicated by the fact that $G_L$ is not differentiable everywhere due to the function $\fracpart{\cdot}$ not being differentiable at multiples of $\tfrac{\pi}{M}$.  This prevents the use of ``standard approaches'' to proving normality that are based on the mean value theorem~\cite{vonMises_diff_stats_1947,vanDerVart1971_asymptotic_stats,Pollard_new_ways_clts_1986,Pollard_conv_stat_proc_1984,Pollard_asymp_empi_proc_1989,van2009empirical}.  However, Lemma~\ref{lem:diffatlambdaL} shows that the derivative $G_L^\prime$ does exist, and is equal to zero, at $\hat{\lambda}_L$.  Similar properties have been used by some of the present authors to analyse the behaviour of polynomial-phase estimators~\cite{McKilliam_LSU_polyest_part_arxiv_2012}.  %\cite{McKilliam_LSU_polyest_part1_2012,McKilliam_LSU_polyest_part2_2012}.
Define the function
\begin{align}\label{eq:RLdef}
R_L(\lambda) &= \frac{1}{\rho_0 L} \Im(Z(\theta))
	\\
...
\]
where $o_P(1)$ denotes a sequence of random variables converging in probability to zero as $L \rightarrow \infty$, and $p,d$ and $H$ are defined in the statements of Theorems~\ref{thm:consistency}~and~\ref{thm:normality}.  Lemma~\ref{lem:empiricprocc} shows that
\[
...
%\section{The Gaussian noise case \comment{-- remove or expand?}}\label{sec:gaussian-noise-case}
%
...
Figs.~\ref{fig:plotphaseBPSK},~\ref{fig:plotphaseQPSK}~and~\ref{fig:plotphase8PSK} show the sample mean square error (MSE) of the phase estimator when $M=2$, $4$, $8$ with $L=4096$ and for varying proportions of pilot symbols $\abs{P} = 0$, $\tfrac{L}{32}$, $\tfrac{L}{8}$, $\tfrac{L}{2}$, $L$.  When $\abs{P} \neq 0$ the mean square error is computed as $\tfrac{1}{T}\sum_{i=1}^T\sfracpart{\hat{\theta}_i - \theta_0}_\pi^2$.  Otherwise, when $\abs{P}=0$ the mean square error is computed as $\tfrac{1}{T}\sum_{i=1}^T\sfracpart{\hat{\theta}_i - \theta_0}^2$ as in~\cite{McKilliam_leastsqPSKnoncoICASSP_2012}.  The dots, squares, circles and crosses are the results of Monte-Carlo simulations with the least squares estimator.  \comment{The Cram\'er-Rao bounds (CRB) for the fully data-aided case with $\abs{P}=L$ are shown as a reference. Note that the MSE curves for the least squares estimator for $\abs{P}=L$ coincide with the CRBs.}  The solid lines are the estimator MSEs predicted by Theorem~\ref{thm:normality}.   %(and by Theorem~2 from~\cite{McKilliam_leastsqPSKnoncoICASSP_2012} when $\abs{P} = 0$).  
The prediction is made by dividing the asymptotic covariance matrix by $L$.  The theorem accurately predicts the behaviour of the phase estimator when $L$ is sufficiently large.  As the SNR decreases the variance of the phase estimator approaches that of the uniform distribution on $[-\pi, \pi)$ when $\abs{P}>0$, and the uniform distribution on $[-\tfrac{\pi}{M}, \tfrac{\pi}{M})$ when $\abs{P}=0$~\cite{McKilliam_leastsqPSKnoncoICASSP_2012}.  As the SNR increases the variance of the estimators converges to that of the estimator where all symbols are pilots, i.e.\ $\abs{P} = L$.
...
Figs.~\ref{fig:plotphaseBPSK},~\ref{fig:plotphaseQPSK}~and~\ref{fig:plotphase8PSK} also display the sample MSE of the non-data aided phase estimator of Viterbi and Viterbi~\cite{ViterbiViterbi_phase_est_1983} described by~\eqref{eq:viterbiviterbi}.  This estimator requires selection of a function $F$ that transforms the amplitude of each sample prior to the final estimation step.  % (see equation (2) in~\cite{ViterbiViterbi_phase_est_1983}).  
...
\comment{Replace variance plot by MSE plot and rewrite/shorten para.}  Fig.~\ref{fig:plotampBPSK} %,~\ref{fig:plotampQPSK}~and~\ref{fig:plotamp8PSK} 
shows the variance of the amplitude estimator when $M=2$ and with $L=32$, $256$, $2048$ and when the number of pilots symbols is $\abs{P} = 0$, $\tfrac{L}{2}$, $L$.  The solid lines are the variance predicted by Theorem~\ref{thm:normality}.  The dots and crosses show the results of Monte-Carlo simulations.  Each point is computed as the unbiased error $\tfrac{1}{T}\sum_{i=1}^T\big(\hat{\rho}_i - \rho_0G(0)\big)^2$.  This requires $G(0)$ to be known.  In practice $G(0)$ may not be known at the receiver, so Fig.~\ref{fig:plotampBPSK} %,~\ref{fig:plotampQPSK}~and~\ref{fig:plotamp8PSK} 
serves to validate the correctness of our asymptotic theory, rather than to suggest the practical performance of the amplitude estimator.  When SNR is large $G(0)$ is close to $1$ and the bias of the amplitude estimator is small.  However, $G(0)$ grows without bound as the variance of the noise increases, so the bias is significant when SNR is small.  %As indicated in Figs.~\ref{fig:plotampBPSK},~\ref{fig:plotampQPSK}~and~\ref{fig:plotamp8PSK} the variance of the least squares amplitude estimator is smaller than that of the estimator when all symbols are pilots.  However, due to the bias, the MSE of the least squares amplitude estimator is \emph{not} smaller than that of the unmodulated carrier.

\comment{Add a plot of the theoretical bias $\rho_0 (G(0)-1)$ and the simulated bias $\tfrac{1}{T}\sum_{i=1}^T(\hat\rho_i-\rho_0)$ as a function of SNR for different values of $L$?}

Fig.~\ref{fig:plotphaseQPSKmultL} shows the MSE of the phase estimator when $M=4$ and $L=32$, $256$, $2048$ and the number of pilots is $\abs{P}=\tfrac{L}{8}$, $L$.  The figure depicts an interesting phenomenon.  When $L=2048$ and $\abs{P} = \tfrac{L}{8} = 256$ the number of pilots symbols is the same as when $L=\abs{P} = 256$.  When the SNR is small (approximately less than \unit[0]{dB}) the least squares estimator using the $256$ pilots symbols and also the $2048-256=1792$ data symbols performs \emph{worse} than the estimator that uses only the $256$ pilots symbols.  A similar phenomenon occurs when $L=256$ and $\abs{P} = \tfrac{L}{8} = 32$.  %When the SNR is small it is better to ignore the data symbols and use only the pilot symbols.  
...
\comment{Add packet error rate results for an ASRP ST1 uplink:
\begin{itemize}
\item
	single user, bursts of 246 QPSK symbols, preamble of 14 pilots, symbol rate 1024Hz, burst duration 250ms, oversampling factor 32
\item
	iterative refinement of estimates using LDPC decoder output (i.e.\ turbo sync)
\item
	genie-aided synchronisation of all other channel parameters? Our current ST1 framework doesn't have this functionality.
\item
	vary number of pilots?
\item
	performance comparison with Viterbi Viterbi initialisation when $\abs{P}=0$ and purely data-aided initialisation when $\abs{P}>0$
\end{itemize}
}
...
\begin{figure}[p]
	\centering
		...
		\caption{Phase error versus SNR for BPSK with $L=4096$. \comment{Add Viterbi Viterbi curve and include data-aided CRB.}}
...
\begin{figure}[p]
	\centering
		...
		\caption{Phase error versus SNR for QPSK with $L=4096$. \comment{Include data-aided CRB.}}
...
\begin{figure}[p]
	\centering
		...
		\caption{Phase error versus SNR for $8$-PSK with $L=4096$. \comment{Include data-aided CRB.}}
...
\begin{figure}[tp]
	\centering
		...
		\caption{Amplitude estimator variance (bias removed) versus SNR for BPSK. \comment{Change to MSE.}}
...
\begin{figure}[tp]
	\centering
		...
		\caption{Phase error versus SNR for QPSK. \comment{Include data-aided CRBs.}}
...
\section{Circularly symmetric complex random variables}\label{apdx:circ-symm-compl}

%Before describing the statistical properties of the least squares estimator, we first require some properties of complex valued random variables. A complex random variable $W$ is said to be \emph{circularly symmetric} if its phase $\angle{W}$ is independent of its magnitude $\abs{W}$ and if the distribution of $\angle{W}$ is uniform on $[0,2\pi)$.  That is, if $Z \geq 0$ and $\Theta \in [0,2\pi)$ are real random variables such that $Ze^{j\Theta} = W$, then $\Theta$ is uniformly distributed on $[0,2\pi)$ and is independent of $Z$.  If the pdf of $Z$ is $f_Z(z)$, then the joint pdf of $\Theta$ and $Z$ is 

Let $Z \geq 0$ and $\Theta \in [0,2\pi)$ be real random variables and let $\Theta$ be uniformly distributed on $[0,2\pi)$ and independent of $Z$.  Then the complex random variable $W=Ze^{j\Theta}$ is said to be \emph{circularly symmetric}.  If the pdf of $Z$ is $f_Z(z)$, then the joint pdf of $\Theta$ and $Z$ is 
\[
...
\begin{align}%\label{eq:expectRepartRphi}
\expect \Re(R e^{j\Phi}) = \expect R \cos(\Phi) = 1,
\notag\\
\label{eq:expectImpartRphi}
\expect \Im(R e^{j\Phi}) = \expect R \sin(\Phi) = 0,
\end{align}
where $\Re(\cdot)$ and $\Im(\cdot)$ denote the real and imaginary parts.
...
\section{Lemmas for the proof of Theorem~\ref{thm:consistency}} \label{apdx:proof-almost-sure}
...
\end{align*}
%The last line converges almost surely to zero by Lemma~\ref{lem:uniflawGGL}. 
\comment{The last line converges almost surely to zero.  This type of result is called a~\emph{uniform law of large numbers} and follows from standard arguments.  See, for example, van~de~Geer~\cite[Ch.~3]{van2009empirical} or Pollard~\citep[Ch.~2]{Pollard_conv_stat_proc_1984}.  A proof of this specific result is given in~\cite[Lemma~2]{McKilliam_leastsqPSKpilotsdata_arxiv}.
}
\end{IEEEproof}
...
\]
Now by Lemma~\ref{lem:convtoexpGlamL}, $\sabs{G_L(\hat{\lambda}_L) - G(\hat{\lambda}_L)} \rightarrow 0$ \comment{almost surely?} as $L \rightarrow \infty$ and $\sabs{G(\hat{\lambda}_L) - G(0)} \rightarrow 0$ almost surely as $L \rightarrow \infty$.
...
\section{Lemmas for the proof of Theorem~\ref{thm:normality}} \label{apex:proof-asymptotic-normality}
...
\end{lemma}
\begin{IEEEproof}
Put $A(\lambda) = \expect R_1 e^{j\fracpart{\Phi_1}} \chi(\Phi_1,\lambda)$.  Recalling from \eqref{eq:pdfRPhi} that $f(r,\phi)$ is the joint pdf of $R_1$ and $\Phi_1$ we have
\begin{align*}
...
\]
From the standard central limit theorem the distribution of $C_L$ converges to the normal with mean $\sqrt{p}\,\expect R_1 \sin(\Phi_1) = 0$ as a result of~\eqref{eq:expectImpartRphi} in Appendix~\ref{apdx:circ-symm-compl}, and variance
\[
