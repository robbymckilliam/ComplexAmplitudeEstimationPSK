...
\] 
is differentiable everywhere except when $\fracpart{\lambda + \Phi_i} = -\tfrac{\pi}{M}$ for any $i \in D$ where $R_i > 0$.  Let $q_i$ be the smallest number from $[-\tfrac{\pi}{M}, 0]$ such that
\[
 \sin(q_i) > -\frac{\rho_o \sabs{\eta}^2 R_k}{4 L \hat{\rho} \sin(\pi/M)}
\]
