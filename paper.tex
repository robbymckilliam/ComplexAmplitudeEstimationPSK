...
\]
Lemma~\ref{lem:convdistGLdash} shows that the distribution of $\sqrt{L}G_L'(0)$ converges to the normal with zero mean and variance
\[
p \prob R_1^2 \sin^2(\Phi_1) + d \prob R_1^2 \sin^2\fracpart{\Phi_1}
\]
and it follows that the distribution of $\sqrt{L}\hat{\lambda}_L$ converges to the normal with zero mean and variance
\[
\frac{p \prob R_1^2 \sin^2(\Phi_1) + d \prob R_1^2 \sin^2\fracpart{\Phi_1}}{?}
\]
as required.  

Define the function $H_L(\lambda) = \prob G_L(\lambda)$.  Using~\eqref{eq:rhoGLZ} we have
\begin{align*}
...
\]
Lemma~\ref{lem:k2} shows that
\[
k_2(\hat{\lambda}_L) = \hat{\lambda}_L \big( ? \; + \; o_P(1) \big).
\]
\end{IEEEproof}

\begin{lemma}\label{lem:k2}
With the function $k_2(\lambda)$ defined as in~\eqref{eq:k2def}, then
\[
k_2(\hat{\lambda}_L) = \hat{\lambda}_L \big( H \; + \; o_P(1) \big).
\]
where
\[
H = 2 \sin(\tfrac{\pi}{M}) \sum_{k = 0}^M g(\tfrac{(2k+1)\pi}{M})
...
\end{lemma}
\begin{IEEEproof}
Now, if $0 \leq \lambda < \tfrac{\pi}{M}$, then for each integer $k$,
\begin{align*}
...
\end{align*}
when $\phi \in [\tfrac{ (2k-1)\pi}{M}, \tfrac{(2k+1)\pi}{M} - \lambda )$ and 
\begin{align*}
...
\end{align*}
when $\phi \in [ \tfrac{(2k+1)\pi}{M} - \lambda,\tfrac{(2k+1)\pi}{M} )$, and where
\[
v_k(\phi) = \sin(\phi - \tfrac{2\pi k}{M}) - \sin(\phi - \tfrac{2\pi (k-1)}{M}).
\]
Observe that $v_k(\tfrac{(2k+1)\pi}{M}) = 2\sin(\tfrac{\pi}{M})$.  We can now write
\[
...
\begin{align*}
\int_{\tfrac{(2k+1)\pi}{M} - \lambda}^{\tfrac{(2k+1)\pi}{M}}v_k(\phi)g(\phi)d\phi &= \lambda( v_k(\tfrac{(2k+1)\pi}{M}) + \nu_k(\lambda) ) \\
