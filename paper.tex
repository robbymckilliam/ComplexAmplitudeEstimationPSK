...
\end{lemma}
\begin{IEEEproof}
Because $T_\epsilon(k)$ is a subset of $B_\epsilon$,
\[
\sup_{\lambda \in T_\epsilon(k)} \sabs{X_L(\lambda)} \leq \sup_{\sabs{\lambda} < \epsilon} \sabs{X_L(\lambda)}.
\]
Let $\lambda^* \in B_\epsilon$ be a maximiser of $\sabs{X_L(\lambda)}$, that is
\[
\sabs{X_L(\lambda^*)} = \sup_{\sabs{\lambda} < \epsilon}\sabs{X_L(\lambda)}.
\]
By definition $T_\epsilon(k)$ contains a point $\lambda_2$ such that $d_2(\lambda^*,\lambda_2) \leq 2^{-k}D_\epsilon$.  Now
\begin{align*}
\sabs{X_L(\lambda^*) - X_L(\lambda_2)} &= \abs{\frac{1}{\sqrt{L}}\sum_{i\in P \cup D}g_i(f_i(\lambda^*) - f_i(\lambda_2))} \\
&\leq  \frac{1}{\sqrt{L}}\sum_{i\in P \cup D}\abs{g_i}\sabs{f_i(\lambda^*) - f_i(\lambda_2)} \\
&\leq \frac{g_\text{max}}{\sqrt{L}} \sum_{i\in P \cup D}\sabs{f_i(\lambda^*) - f_i(\lambda_2)} \\
&\leq \frac{g_\text{max}}{\sqrt{L}} d(\lambda^*,\lambda_2) \\
&\leq \frac{g_\text{max}}{\sqrt{L}} 2^{-k} D_\epsilon.
\end{align*}
where $g_\text{max} = \max_{i\in P\cup D} \abs{g_i}$.
\end{IEEEproof}
...
\end{lemma}
\begin{IEEEproof}
Let $b_k(\lambda)$ maps each $\lambda \in T_\epsilon(k)$ to an element in $T_\epsilon(k-1)$ such that 
\[
d(\lambda, b_k(\lambda)) \leq 2^{1-k}C_\epsilon.
\]
The existance of $b_k$ is guaranteed by the definition of $T_\epsilon(k-1)$. 
\end{IEEEproof}
