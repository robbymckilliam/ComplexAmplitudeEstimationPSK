...
\[
\fracpart{x} = x - \frac{2\pi}{M}\operatorname{round}\left(\frac{Mx}{2\pi}\right).
\]

\begin{theorem}\label{thm:consistency}
Let $\{w_i\}$ be a sequence independent and identically distributed, zero mean, circularly symetric complex random variables.  Let $R_i > 0$ and $\Phi_i \in [-\pi,\pi)$ be random variables satisfying
\[
...
\] 
as $L \rightarrow \infty$ for positive real numbers $p$ and $d$ and if $h_2(x)$ is uniquely maximised at $x = 0$ over the interval $[\tfrac{-\pi}{M},\tfrac{-\pi}{M})$, then:
\begin{enumerate}
\item $\sfracpart{\theta_0 - \hat{\theta}}_\pi \rightarrow 0$ almost surely as $L \rightarrow \infty$,
\item $\hat{\rho} \rightarrow \rho_0 \big( p h_1(0) + d h_2(0) \big)$ almost surely as $L \rightarrow \infty$.
\end{enumerate}
\end{theorem}
...
\]
Put $\lambda_L = \sfracpart{\theta_0 - \hat{\theta}}_\pi$ and $m_L = \hat{\rho} - \rho_0\big(p h_1(0) + d h_2(0) \big)$. If the periodic function $g\fracpart{\phi}_{\pi}$ is continuous at multiples of $\tfrac{\pi}{M}$, then the vector $[\sqrt{L}\lambda_L, \sqrt{L}m_L]$ converges to the bivariate normal distribution with zero mean and covariance
\[
C = ?
\]
...
\[
\rho = \frac{Z + Z^*}{2L} = \frac{1}{L}\Re(Z)
\]
where $Z = \sum_{i\in P \cup D}y_i e^{-j\theta} s_i^*$ and $\Re(\cdot)$ denotes the real part.  Observe that $Z$ is really a function of $\theta$ and $\{d_i, i \in D\}$, but we have supressed the arguments for notational clarity.  We can now write
\[
...
\[
SS(\theta, \{d_i, i \in D\}) = A - \frac{1}{L}\Re(Z)^2.
\]
...
Because the noise $\{w_i\}$ is circularly symetric then
\begin{align*}
y_i &= a_0 s_i + w_i \\
...
\] 
Let $S_L(\lambda) = L^{-2}\rho_0^{-2}\Re(Z(\theta))^2$ and put $\lambda_L = \langle\theta_0 - \hat{\theta}\rangle$.  Then $\lambda_L$ is the maximiser of $S_L(\lambda)$.  We will show that $\lambda_L$ converges almost surely to zero as $L \rightarrow \infty$.  From this, the proof of strong consistency follows.

Put $G_L(\lambda) = \Re(Z(\theta))$ and put
\[
G(\theta) = \lim_{L \rightarrow\infty} \prob G_L(\lambda) = p h_1(\lambda)   +  d h_2(\lambda).
\]
Lemma~\ref{lem:Zconunif} shows that,
\[
\sup_{\lambda \in [-\pi, \pi)}\abs{G_L(\lambda) - \prob G_L(\lambda)} \rightarrow 0
\]
as $N\rightarrow 0$.  As a result of the theorem statement that $G(\lambda)$ is uniquely maximised at $0$ and therefore 
\[
0 \leq G(\lambda_L) - G(0).
\]
Since $\lambda_L$ is the maximiser of $G_L$ then
\[
...
\end{align*}
and because of~\eqref{eq:supELNLN} the last line converges almost surely to zero as $L \rightarrow \infty$ and therefore $G(\lambda_L) \rightarrow G(0)$ almost surely. To complete the proof of strong consistency we use the device of Jenrich~\cite{Jenrich_asymp_nonlin_ulln_1969}. We use the
...
Let $G_L(\lambda) = \re(Z(\theta)$.  Then
\[
\sup_{\lambda \in [-\pi, \pi)}\abs{G_L(\lambda) - G(\lambda)} \rightarrow 0
\]
as $L\rightarrow 0$.
\end{lemma}
