...
Here, consider a slightly more general model than is usual, one that allows the utilisation of pilot symbols, i.e., \emph{coherent detection}.  Our model includes the noncoherent case by setting the number of pilot symbols to zero, that is, putting $P = \emptyset$.
...
We are interested in analysing $\hat{\theta}$ the maximiser of $\abs{Z(\theta)}^2$.  Put $\lambda_L = \fracpart{\theta_0 - \hat{\theta}}$.  We want to show that $\lambda_L$ converges to zero as the number of transmitted symbols $L$ increases to $\infty$ where $\fracpart{x}$ denotes $x$ taken `modulo $2\pi$' into the interval $[-\pi, \pi)$, that is
\[
\fracpart{x} = x - 2\pi\round{\frac{x}{2\pi}}.
\]
...
\[
\hat{u}_i(\theta) = \round{\frac{M}{2\pi}(\angle{y_i} - \theta)} = \round{\frac{M}{2\pi}(\lambda + \Phi_i + k_i)}
\]
where $\lambda = \fracpart{\theta_0 - \theta}$.  So,
\begin{align*}
 y_i e^{-j\theta} \hat{d}_i(\theta)^* &= \rho_j \exp{j \left(\lambda + \Phi_i + k_i - \frac{2\pi}{M}\round{\frac{M}{2\pi}(\lambda + \Phi_i + k_i)}\right)} \\
&= \rho_j \exp{j \left(\lambda + \Phi_i - \frac{2\pi}{M}\round{\frac{M}{2\pi}(\lambda + \Phi_i) }\right)}
\end{align*}
when $i \in D$ and
\[
y_i e^{-j\theta} p_i^* = \rho_j \exp j \left(\lambda + \Phi_i )
\]
when $i \in P$.  Now we can write
\[
Z(\theta) = \sum_{i \in P} \rho_j e^{j(\lambda + \Phi_i)} + \sum_{i \in D}  \rho_j \exp{j \left( \lambda + \Phi_i - \frac{2\pi}{M}\round{\frac{M}{2\pi}(\lambda + \Phi_i)}\right)}.
\]
Put $S_L(\lambda) = \frac{1}{L}\abs{Z(\theta)}^2$ and put $\lambda_L = \fracpart{\theta_0 - \hat{\theta}}$.  Then $\lambda_L$ is the maximiser of $S_L(\lambda)$.  We will show that $\lambda_L$ converges almost surely to zero as $L \rightarrow \infty$.  From this, the proof of strong consistency follows.

BLERG  Show $E S_L$ is maximised uniquely at 0, proof follows.

We now prove the asymptotics normality of the estimator.  At the same time we are able to describe the distribution of the amplitude estimator $\hat{\rho}$.  BLERG, first show that it is differentiable at the truth.
