...
\[
h_1(x) = \prob R_1 \cos(x + \Phi_1) \;\;\; \text{and} \;\;\; h_2(x) =  \prob R_1 \cos\sfracpart{ x + \Phi_1}.
\]
...
\item $\hat{\rho} \rightarrow \rho_0( p h_1(0) + d h_2(0) )$ almost surely as $L \rightarrow \infty$,
\item Put $\lambda_L = \sfracpart{\theta_0 - \hat{\theta}}_\pi$ and $m_L = \hat{\rho} - \rho_0(p h_1(0) + d h_2(0) )$.  Then the vector $[\sqrt{L}\lambda_L, \sqrt{L}m_L]$ converges to the bivariate normal distribution with zero mean and covariance
\[
C = ?
\]
...
\[
y_i = a_0 s_i + w_i = s_i e^{j \theta_0}( \rho_0 + w_i s_i^* e^{-j \theta_0}) = R_i e^{j ( \Phi_i + \theta_0 + \angle{s_i}) }
\] 
...
\begin{align*}
 y_i e^{-j\theta} \hat{d}_i(\theta)^* &= R_i e^{j(\lambda + \Phi_i + \angle{s_i} - \round{\lambda + \Phi_i + \angle{s_i}})} \\
&= R_i e^{j(\lambda + \Phi_i - \round{\lambda + \Phi_i})} \\
&= R_i e^{j\sfracpart{\lambda + \Phi_i}}
\end{align*}
since $\round{x + \angle{s_i}} = \round{x} + \angle{s_i}$ as a result of $\angle{s_i}$ being a multiple of $\tfrac{2\pi}{M}$.  When $i \in P$,  
\[
y_i e^{-j\theta} p_i^* = R_i e^{j(\lambda + \Phi_i)}
\]
Now we can write
\begin{align}
Z(\theta) &= \sum_{i \in P} R_i e^{j(\lambda + \Phi_i)} + \sum_{i \in D} R_i e^{j\sfracpart{\lambda + \Phi_i}}
\end{align}
and
\[
\Re(Z(\theta)) = \sum_{i \in P} R_i \cos(\lambda + \Phi_i) + \sum_{i \in D} R_i \cos(\sfracpart{\lambda + \Phi_i})
\] 
Let $S_L(\lambda) = \frac{1}{L^2}\Re(Z)^2$ and put $\lambda_L = \langle\theta_0 - \hat{\theta}\rangle$.  Then $\lambda_L$ is the maximiser of $S_L(\lambda)$.  We will show that $\lambda_L$ converges almost surely to zero as $L \rightarrow \infty$.  From this, the proof of strong consistency follows.
