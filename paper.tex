...
\end{equation}
Observe that $a_k(\lambda)$ and $b_k(\lambda)$ are functions that converge to zero as $\lambda$ goes to zero, whilst $c_k(\lambda)$ converges to 
\[
...
&= \lambda \left( C_k(\lambda)  + A_k(\lambda) + B_k(\lambda) \right).
\end{align*}
where 
\[
A_k(\lambda) = \sum_{k=0}^{M-1} a_k(\lambda), \;\;\; B_k(\lambda) = \sum_{k=0}^{M} b_k(\lambda), \;\;\; \text{and},
\]
\[
C_k(\lambda) = \sum_{k=0}^{M-1} c_k(\lambda).
\]
Observe both $A_k(\lambda)$ and $B_k(\lambda)$ converges to zero as as $\lambda$ goes to zero, and $C_k(\lambda)$ converges to $H_1$ as $\lambda$ converges to zero.  Put
\[
\gamma(\lambda) = A_k(\lambda) + B_k(\lambda) + C_k(\lambda) - H_1.
\]
Then $\gamma(\lambda)$ converges to zero as $\lambda$ goes to zero and $W(\lambda) = \lambda(H_1 + \gamma(\lambda))$.  The proof follows.
\end{IEEEproof}
