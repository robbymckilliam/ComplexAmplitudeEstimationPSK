...
\]
\[
A_1 = \expect R_1^2\sin^2(\Phi_1), \qquad A_2 = \var\big( R_1^2\sin^2\fracpart{\Phi_1} \big),
\]
\[
...
\[
\expect (\rho_0 - \hat{\rho})^2 = \frac{p B_1 + d B_2}{L} + \rho_0^2(1 - G(0))^2 + o(L^{-1/2}).
\] 
\end{corollary}
...
\[
\sup_{\lambda \in [-\pi, \pi)}\sabs{T(\lambda) - G(\lambda)} \leq o(1) \expect R_1 \rightarrow 0
\]  
...
&=  \abs{G_L(\lambda) - T(\lambda) - G_L(\lambda_n) + T(\lambda_n)} \\
...
\] 
is differentiable everywhere except when $\fracpart{\lambda + \Phi_i} = -\tfrac{\pi}{M}$ and $R_i > 0$ for any $i \in D$.  Let $q$ be the largest negative real number such that
\[
\sin(q) > -\frac{\rho_o \sabs{\eta}^2 R_{\text{min}}}{4 L \hat{\rho} \sin(\pi/M)}
\]
where $\eta = e^{-j2\pi/M} - 1$ and $R_{\text{min}}$ is the smallest strictly postive value of $R_i,$ for $i \in D$, that is
\[
R_{\text{min}} = \min_{i \in D, R_i \neq 0} R_i.
\]  
Lemma~\ref{lem:fracpartlambdahatnotpi} shows that,
\[
\sabs{\sfracpart{\hat{\lambda}_L + \Phi_i}} \leq \frac{\pi}{M} + q
\]
for all $i \in D$.  Thus, $\sfracpart{\hat{\lambda}_L + \Phi_i} \neq \tfrac{\pi}{M}$ if $R_i > 0$.
\end{IEEEproof}

\begin{lemma}\label{lem:fracpartlambdahatnotpi}
\[
\sabs{\sfracpart{\hat{\lambda}_L + \Phi_i}} \leq \frac{\pi}{M} - \frac{\pi}{2LM ?}
\]
for all $i \in D$.
\end{lemma}
\begin{IEEEproof}
Recall that 
\[
\hat{d}_i = \hat{d}_i(\hat{\theta}) = e^{j\round{\angle y_i - \hat{\theta}}}, \;\;\; i \in D
\] 
defined in \eqref{eq:hatdfinxtheta} are the minimsers of the function 
\[
...
\sfracpart{\hat{\lambda}_L + \Phi_i} > \frac{\pi}{M} - ?
\end{equation}
for some $k$ for which $\abs{R_k} > 0$.  Recalling the notation $e_k$ defined in Section~\ref{sec:least-squar-estim}, put $r_i = \hat{d}_i e_k$, then
\begin{align*}
...
&= A - \frac{1}{L} \abs{ \hat{Y} + \eta y_k\hat{d}_k^* }^2 \\
&= A - \frac{1}{L}\abs{\hat{Y}}^2 - \frac{2}{L}\Re\left(\eta \hat{Y}^* y_k \hat{d}_k^* \right) -  \frac{1}{L}\abs{ \eta y_k}^2\\
&= SS(\{\hat{d}_i, i \in D\}) - D
\end{align*}
where 
\[
D = \frac{2}{L}\Re\left(\eta \hat{Y}^* y_k \hat{d}_k^* \right) +  \frac{1}{L}\abs{ \eta y_k }^2.
\]
...
so that
\[
D = 2 \hat{\rho} \rho_0 R_k \Re\left( \eta e^{j\sfracpart{\hat{\lambda}_L + \Phi_k}}\right) + \frac{1}{L}\abs{\eta}^2\rho_0^2 R_k^2.
\]
...
\end{align*}
Because we assumed~\eqref{eq:fraclambassumption},
\[
0 > v > -?
\]
and therefore
\begin{align*}
0 &> 2 \sin(\tfrac{\pi}{M}) \sin(v) \\
&> -\frac{\rho_0 R_k \abs{\eta}^2}{2L\hat{\rho}}
\end{align*}

So $D$ is positive.  But then $SS(\{r_i, i \in D\}) < SS(\{\hat{d}_i, i \in D\})$, violating the fact that $\{\hat{d}_i, i \in D\}$ are minimisers of $SS$.
...
\end{lemma}
\begin{IEEEproof}
As $\hat{\lambda}_L \rightarrow 0$ almost surely as $L\rightarrow\infty$, it is only the behaviour of $k_2(\lambda)$ around $\lambda = 0$ that is relevant.  We will analyse $k_2(\lambda)$ for $0 \leq \lambda < \tfrac{\pi}{M}$.  An analagous argument follows when $-\tfrac{\pi}{M} < \lambda < 0$.  To keep our notation clean put
\[
...
\]
Observe that both $s_k$ and $\eta_k$ are continuous and that $s_k(\psi_{k}) = -2\sin(\tfrac{\pi}{M})$ and $\eta_k(\lambda,\psi_k) = \zeta_{k+1}(\lambda)$.
...
\end{align}
Now, when $0 < k < M$, we have $0 < \psi_{k-1} < \psi_k < 2\pi$, and so, using~\eqref{eq:vk},
\begin{align*}
