...
\]
where $o_P(1)$ denotes a number converging in probablity to zero as $L \rightarrow \infty$.  The constant real numbers $p,d$ and $H$ are defined in the statement of Theorems~\ref{thm:consistency}~and~\ref{thm:normality}.  Lemma~\ref{lem:empiricprocc} shows that
\[
...
We now analyse the asymptotic distribution of $\sqrt{L} m_L$.  Define the function $T(\lambda) = \expect G_L(\lambda)$.  Using~\eqref{eq:rhoGLZ},
\begin{align*}
...
\end{align*}
and by a Taylor expansion of $\sin(\lambda + \Phi_1 - \tfrac{2\pi}{M}k)$ about $\Phi_1 - \tfrac{2\pi}{M}k$ we obtain 
\begin{align*}
...
\begin{align*}
\sin&\fracpart{\lambda + \Phi_1} - \sin\fracpart{\Phi_1} \\
&=  s_k(\Phi_1) + \lambda\big(\cos(\Phi_1 - \tfrac{2\pi}{M}k - \tfrac{2\pi}{M}) + \zeta_{k+1}(\lambda)\big) 
\end{align*}
where
\[
...
\end{align*}
Define the function $W(\lambda) = \expect R_1 v(\lambda, \Phi_1)$ so that $k_2(\lambda) = \lambda h_2(0) + W(\lambda).$  Lemma~\ref{lem:expRvlamphi} shows that $W(\lambda) =  \lambda(H_1 + \gamma(\lambda))$ where $\gamma(\lambda)$ is a function that goes to zero as $\lambda$ goes to zero, and
\[
H_1 = - 2 \sin(\tfrac{\pi}{M}) \sum_{k = 0}^{M-1} g(\psi_k) = H - h_2(0).
\]
Because $\hat{\lambda}_L$ converges almost surely to zero as $L \rightarrow\infty$, then $\gamma(\hat{\lambda}_L)$ converges almost surely (and therefore also in probability) to zero as $L \rightarrow\infty$, i.e. $\gamma(\hat{\lambda}_L) = o_P(1)$, and therefore 
\[
...
$W(\lambda) =  \lambda(H_1 + \gamma(\lambda))$ where $\gamma(\lambda)$ is a function that goes to zero as $\lambda$ goes to zero.
\end{lemma}
\begin{IEEEproof}
In this lemma, as Lemma~\ref{lem:k2conv}, we are working under the assumption that that $0 \leq \lambda < \frac{\pi}{M}$.  An analgous argument follows when $-\tfrac{\pi}{M} \leq \lambda < 0$.  Recalling that $f(r,\phi)$ is the joint probability density function of $R_1$ and $\Phi_1$, then,
\begin{align*}
W(\lambda) &= \expect R_1 v(\lambda, \Phi_1) = \int_0^{2\pi}\int_{0}^\infty r v(\lambda, \phi) f(r,\phi) dr d\phi \\
&= \int_0^{2\pi} v(\lambda, \phi) g(\phi) d\phi,
\end{align*}
and, since $v(\lambda, \phi) = \sum_{k\in\ints} v_k(\lambda, \phi)$,
\begin{equation}\label{eq:Winfsum}
W(\lambda) = \sum_{k \in \ints} \int_0^{2\pi} v_k(\lambda, \phi) g(\phi) d\phi.
\end{equation}
...
&= \int_{\psi_{k} - \lambda}^{\psi_{k}} (s_k(\phi) + \lambda\eta_k(\lambda, \phi)) g(\phi) d\phi \\
&\hspace{1cm} +  \int_{\psi_{k-1}}^{\psi_{k} - \lambda}\lambda\zeta_k(\lambda)g(\phi) d\phi.
\end{align*}
When $k = 0$, we have $\psi_{k-1} < 0 < \psi_k - \lambda < \psi_k < 2\pi$, so that
\begin{align*}
\int_0^{2\pi} v_k(\lambda, \phi) g(\phi) d\phi &= \int_{0}^{\psi_{k}} v_k(\lambda, \phi) g(\phi) d\phi \\
&= \int_{\psi_{k} - \lambda}^{\psi_{k}} (s_k(\phi) + \lambda\eta_k(\lambda, \phi)) g(\phi) d\phi \\
&\hspace{1cm} +  \int_{0}^{\psi_{k} - \lambda}\lambda\zeta_k(\lambda)g(\phi) d\phi ,
\end{align*}
and when $k = M-1$, we have $0 < \psi_{k-1} < 2\pi < \psi_k - \lambda$, so that
\begin{align*}
\int_0^{2\pi} v_k(\lambda, \phi) g(\phi) d\phi &= \int_{\psi_{k-1}}^{2\pi} v_k(\lambda, \phi) g(\phi) d\phi \\
&= \int_{\psi_{k-1}}^{2\pi}\lambda\zeta_k(\lambda)g(\phi) d\phi.
\end{align*}
Otherwise, when $k < 0$ or $k > M$, 
\[
\int_0^{2\pi} v_k(\lambda, \phi) g(\phi) d\phi = 0
\]
So the infinite sum in~\eqref{eq:Winfsum} is actually a finite sum,
\begin{align*}
W(\lambda) &= \sum_{k=0}^{M} v_k(\lambda, \phi) g(\phi) d\phi \\
&= \sum_{k=0}^{M-1}\int_{\psi_{k} - \lambda}^{\psi_{k}} (s_k(\phi) + \lambda\eta_k(\lambda, \phi)) g(\phi) d\phi \\
&\hspace{0.5cm} + \sum_{k=0}^{M-1} \int_{\psi_{k-1}}^{\psi_{k} - \lambda}\lambda\zeta_k(\lambda)g(\phi) d\phi \\
&\hspace{0.5cm} +  \int_{0}^{\psi_{k} - \lambda}\lambda\zeta_k(\lambda)g(\phi) d\phi + \int_{\psi_{k-1}}^{2\pi}\lambda\zeta_k(\lambda)g(\phi) d\phi + 
\end{align*}
\begin{align*}
