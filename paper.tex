...
\]
Put $\hat{\lambda}_L = \sfracpart{\theta_0 - \hat{\theta}}_\pi$ and $\hat{m}_L = \hat{\rho} - \rho_0 G(0)$. %If $g(\phi)$ is continuous at $\phi = (2k+1)\tfrac{\pi}{M}$ for each $k = 0, 1, \dots M-1$, Then 
...
\[
\eta_k(\lambda, \Phi_1) = c_k(\Phi_1) + \zeta_{k+1}(\lambda).
\]  
Observe that both $s_k$ and $c_k$ are continuous functions and that $s_k(\psi_{k}) = -2\sin(\tfrac{\pi}{M})$ and $c_k(\psi_{k}) = 0$.  %It follows, since $\zeta_{k+1}$ is also continuous, that the function $\eta_k(\lambda, \Phi_1)$ is continuous in both $\lambda$ and $\Phi_1$, and $\eta_k(0, \psi_k) = 0$.
...
\end{align*}
Define the function $W(\lambda) = \expect R_1 v(\lambda, \Phi_1)$ so that $k_2(\lambda) = \lambda h_2(0) + W(\lambda).$  Lemma~\ref{lem:expRvlamphi} shows that $W(\lambda) =  \lambda(H_1 + \gamma(\lambda))$ where $\gamma$ is a continuous function with $\gamma(0) = 0$, and
\[
...
\end{lemma}
\begin{IEEEproof}
In this lemma, as in Lemma~\ref{lem:k2conv}, we are working under the assumption that that $0 \leq \lambda < \frac{\pi}{M}$.  An analgous argument follows when $-\tfrac{\pi}{M} \leq \lambda < 0$.  Recalling that $f(r,\phi)$ is the joint probability density function of $R_1$ and $\Phi_1$, then,
\begin{align*}
...
\end{align*}
and, since $v(\lambda, \phi) = \sum_{k\in\ints} v_k(\lambda, \phi)$, then,
...
\end{align*}
where, for each integer $k = 1, \dots, M-1$, the functions $a_k$, $b_k$, and $c_k$ are
...
\end{equation}
Observe that $a_k(\lambda)$ and $b_k(\lambda)$ are functions that converge to zero as $\lambda$ goes to zero from above.  Since $g$ is continuous at $\psi_k$ then $c_k(\lambda)$ converges to 
\begin{equation}\label{eq:skconverge}
...
We are only interested in the limits from above because we are working under the assumption that that $0 \leq \lambda < \frac{\pi}{M}$.  The analgous argument when $-\tfrac{\pi}{M} \leq \lambda < 0$ would involve limits as $\lambda$ approaches zero from below.
...
\]
Observe, that the function $A(\lambda)$ and $B(\lambda)$ converge to to zero as as $\lambda$ approaches zero from above, and, using~\eqref{eq:skconverge}, the function $C(\lambda)$ converges to 
\[
H_1 =  - 2 \sin(\tfrac{\pi}{M}) \sum_{k = 0}^{M-1} g(\tfrac{2\pi}{M}k + \tfrac{\pi}{M})
\]
as $\lambda$ converges to zero from above.  Put
\[
\gamma(\lambda) = A(\lambda) + B(\lambda) + C(\lambda) - H_1.
\]
Then $\gamma(\lambda)$ converges to zero as $\lambda$ approaches to zero from above and $W(\lambda) = \lambda(H_1 + \gamma(\lambda))$.  The proof follows.
\end{IEEEproof}
