...
We have
\[
Q_L(\lambda) = \expect R_L(\lambda) - \expect R_L(0) = \tfrac{\abs{P}}{L} k_1(\lambda) + \tfrac{\abs{D}}{L} k_2(\lambda)
\]
where
\begin{align}
...
\begin{align*}
Q_L(\hat{\lambda}_L) &=  \tfrac{\abs{P}}{L} \hat{\lambda}_L \big( 1  + o_P(1)\big)  + \tfrac{\abs{D}}{L} \hat{\lambda}_L ( H + o_P(1) ) \\
...
\begin{lemma}\label{lem:empiricprocc}
\[
\sqrt{L}\big( R_L(\hat{\lambda}_L) - Q_L(\hat{\lambda}_L) \big) = o_P(1) + \sqrt{L} R_L(0).
\]
\end{lemma}
...
Put 
\[
Z_L(\lambda) = \sqrt{L}\big( R_L(\lambda) - \expect R_L(\lambda) - R_L(0) + \expect R_L(0)\big)
\] 
so that
\[
\sqrt{L}\big( R_L(\hat{\lambda}_L) - Q_L(\hat{\lambda}_L) \big) = Z_L(\hat{\lambda}_L) + \sqrt{L} R_L(0)
\]
Lemma~? shows that for any $\delta > 0$ and $\nu > 0$, there exists $\epsilon > 0$ such that
\[
\Pr\left\{ \sup_{\sabs{\lambda}<\epsilon}\sabs{Z_L(\lambda)} > \delta  \right\} < \nu
\]
...
\begin{align*}
  \prob&\left\{\sabs{ Z_L(\hat{\lambda}_L) } > \delta \right\} \\
&= \prob\left\{ \sabs{Z_L(\hat{\lambda}_L)} > \delta \;\text{and} \; \sabs{\hat{\lambda}_L} < \epsilon \right\} \\
&\hspace{0.7cm} + \prob\left\{ \sabs{Z_L(\lambda_L)} > \delta  \;\text{and} \; \sabs{\hat{\lambda}_L} \geq \epsilon \right\} \\
&\leq \prob\left\{  \sup_{\lambda < \epsilon} \sabs{ Z_L(\lambda) } > \delta \right\} + \prob\left\{ \sabs{\hat{\lambda}_L} \geq \epsilon \right\} \\
&\leq 2\nu.
\end{align*}
for all sufficiently large $L$.  Since $\nu$ and $\delta$ can be chosen arbitrarily small, it follows that $Z_L(\hat{\lambda}_L)$ converges in probability to zero as $N\rightarrow\infty$.
\end{IEEEproof}
