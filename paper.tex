...
\]
The mean of $R e^{j\Phi}$ is equal to one because the mean of $X$ is zero.



\section{Statistical properties of the least square estimator}
...
where
\[
h_1(x) = \prob R_1 \cos(x + \Phi_1), \;\;\; h_2(x) =  \prob R_1 \cos\sfracpart{ x + \Phi_1}.
\]
...
\]
Put $\lambda_L = \sfracpart{\theta_0 - \hat{\theta}}_\pi$ and $m_L = \hat{\rho} - \rho_0 G(0)$. If $g(\phi)$ is continuous at $\phi = (2k+1)\tfrac{\pi}{M}$ for every integer $k$, then the distribution of $(\sqrt{L}\lambda_L, \sqrt{L}m_L)$ converges to the bivariate normal with zero mean and covariance
\[
...
and
\[
A_1 = \prob R_1^2\sin^2(\Phi_1), \qquad A_2 = \prob R_1^2\sin^2\fracpart{\Phi_1}.
\]
\end{theorem}
...
\]
and $Z^*(\theta)$ is the conjugate of $Z(\theta)$.  Differentiating with respect to $\rho$ and setting the resulting expression to zero gives 
...
It remains to prove part two of this theorem regarding the convergence of the amplitude estimator $\hat{\rho}$.  From~\eqref{eq:hatrhoZ} the amplitude estimator satisfies 
\begin{equation}\label{eq:rhoGLZ}
\hat{\rho} = \frac{1}{L}\Re(Z(\theta)) = \rho_0 G_L(\hat{\lambda}_L).
\end{equation}  
...
\end{align*}
Define the function $Q(\lambda) = \prob G_L^\prime(\lambda) - \prob G_L^\prime(0)$ and write
\begin{align*}
...
We now analyse the asymptotic distribution of $\sqrt{L} m_L$.  Define the function $H_L(\lambda) = \prob G_L(\lambda)$.  Using~\eqref{eq:rhoGLZ} we have
\begin{align*}
...
\]
Lemma~\ref{lem:XL} shows that the distrubtion of $X_L$ converges to the normal with zero mean and covariance
\[
?
\]
...
Write
\[
Q(\lambda) = \prob G_L^\prime(\lambda) - \prob G_L^\prime(0) = \frac{\abs{P}}{L} k_1(\lambda) + \frac{\abs{D}}{L} k_2(\lambda)
\]
where
\begin{align}
k_1(\lambda) &= \prob R_1\big( \sin(\lambda + \Phi_1) - \sin(\Phi_1) \big), \;\;\; \text{and} \nonumber\\
k_2(\lambda) &= \prob R_1\big( \sin\fracpart{\lambda + \Phi_1} - \sin\fracpart{\Phi_1} \big). \label{eq:k2def}
\end{align}
Since,
\[
...
\begin{align*}
k_1(\lambda) &= \prob R_1\lambda(\cos(\Phi_1) \; + \; \zeta(\lambda)) \\
&= \lambda \big( \prob R_1\cos(\Phi_1) \; + \; \zeta(\lambda) \prob R_1 \big) \\
&= \lambda \big( 1 \; + \; \zeta(\lambda) K \big),
\end{align*}
since $h_1(0) = \prob R_1\cos(\Phi_1) = 1$ and where we put $K = \prob R_1$, a finite constant.  Because $\hat{\lambda}_L$ converges almost surely to zero as $L \rightarrow\infty$, then $\zeta(\hat{\lambda}_L)$ converges almost surely (and therefore also in probability) to zero as $L \rightarrow\infty$, i.e. $\zeta(\hat{\lambda}_L) = o_P(1)$.  So, 
\[
...
Now
\begin{align*}
k_2(\lambda) &=  \prob R_1\big( \sin\fracpart{\lambda + \Phi_1} - \sin\fracpart{\Phi_1} \big) \\
&= \prob R_1\big( v(\lambda, \Phi_1) + \lambda\cos\fracpart{\Phi_1} \big) \\
&= \prob R_1 v(\lambda, \Phi_1) + \lambda h_2(0).
\end{align*}
It remains to evaluate $\prob R_1 v(\lambda, \Phi_1)$.  Recalling that $f(r,\phi)$ is the joint probability density function of $R_1$ and $\Phi_1$,
\begin{align*}
