...
&\leq 2\sup_{x \in [-\pi, \pi)} \sabs{G(x) - G_L(x)},
\end{align*}
...
\[
\sup_{x \in [-\pi, \pi)}\sabs{G_L(x) - G(x)} \rightarrow 0
\] 
almost surely as $L \rightarrow \infty$.
\end{lemma}
\begin{IEEEproof}
Put $T(x) = \expect G_L(x)$ and observe that
\begin{align*}
T(\lambda) - G(\lambda) &= \big(\tfrac{\abs{P}}{L} - p\big)h_1(\lambda) + \big(\tfrac{\abs{D}}{L} - d\big)h_2(\lambda) \\
&= o(1) h_1(\lambda) + o(1) h_2(\lambda)
\end{align*}
where $o(1)$ denotes a number converging to zero as $L \rightarrow \infty$.  Since $\sabs{h_1(\lambda)} \leq 1$ and $\sabs{h_2(\lambda)} \leq 1$ for all $\lambda \in [-\pi, \pi)$, then $\sup_{\lambda \in [-\pi, \pi)}\sabs{T(\lambda) - G(\lambda)} \rightarrow 0$ as $L\rightarrow \infty$.  Now
\begin{align*}
...
\end{align*}
and Lemma~\ref{lem:uniflawTGL} shows that $\sup_{\lambda \in [-\pi, \pi)}\sabs{G_L(\lambda) - T(\lambda)} \rightarrow 0$ as $L\rightarrow \infty$.
\end{IEEEproof}


\begin{lemma}\label{lem:uniflawTGL} 
Put $T(x) = \expect G_L(x)$, then
\[
\sup_{x \in [-\pi, \pi)}\sabs{G_L(x) - T(x)} \rightarrow 0
\] 
almost surely as $L \rightarrow \infty$.
\end{lemma}
\begin{IEEEproof}
