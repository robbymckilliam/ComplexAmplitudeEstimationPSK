...
\expect_{R,\Phi} \sup_{\sabs{\lambda} < \epsilon} \abs{ X_L(\psibf) } \leq K_1 \sqrt{C_\epsilon(\{\Phi\})}
\end{equation}
where $K_1$ is a finite, positive constant, and $C_\epsilon(\{\Phi\})$ is average number of times $\Phi_i, i \in D$ are inside the set
\[
S_\epsilon = \bigcup_{k \in \ints} [\psi_k - \epsilon, \psi_k + \epsilon],
\]
where $\psi_k = \tfrac{2\pi}{M}k + \tfrac{\pi}{M}$.  That is,
\[
C_\epsilon(\{\Phi\}) = \frac{1}{L} \sum_{i \in P \cup D} I_\epsilon(\Phi_i)
\]
where $I_\epsilon(\Phi_i) = 1$ if $\Phi_i \in S_\epsilon$ and zero otherwise.
\end{IEEEproof}
...
For notational convenience we use the abbreviations 
\[
C_\epsilon = C_\epsilon(\{\Phi\}) \qquad \text{and} \qquad f_{i}(\lambda) = f_{i}(\lambda, R_i, \Phi_i)
\]
...
\[
\expect_{R,\Phi} \sup_{\sabs{\lambda} < \epsilon} \abs{ X_L(\lambda) } \leq K_1 \sqrt{C_\epsilon}.
\]
\end{lemma}
\begin{IEEEproof}
Let $B_\epsilon = \{x \in \reals \mid \abs{x} < \epsilon\}$ be the set of real numbers with magnitude less than $\epsilon$.  For each non negative integer $k$, let $T_\epsilon(k)$ be a discrete subset of $\reals$ with the property that for every $\lambda \in B_\epsilon$ there exists $\lambda^* \in T_\epsilon(k)$ such that the pseudometric
\[
d(\lambda, \lambda^*) = \sum_{i \in P \cup D} \big( f_{i}(\lambda) - f_{i}(\lambda^*) \big)^2 \leq 2^{-k} C_\epsilon N.
\]
We specifically define $T_\epsilon(0)$ to contain only zero.  Defined this way 
\end{IEEEproof}
