...
Figures~\ref{fig:plotphaseBPSK},~\ref{fig:plotphaseQPSK}~and~\ref{fig:plotphase8PSK} also display the non-data aided Cram\'{e}r-Rao lower bound (CRB) for carrier phase given in~\cite{Cowley_crbs_phase_freq_1996,950345}.  These CRBs are derived for the non-data aided setting and are only valid for estimators that do not make use of pilots symbols, i.e., $\abs{P}=0$.
...
%Thus, when the SNR is small it is better to ignore the data symbols and use only the pilot symbols.   
 %This behaviour suggests modifying the objective function to give the pilots symbols more importance when the SNR is low.  For example, rather than minimise~\eqref{eq:LSdefn} we could instead minimise a weighted version of it,
 % \[
