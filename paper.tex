...
\]
Both $A(\lambda)$ and $B(\lambda)$ converge to zero as $\lambda \rightarrow 0$, whilst $C(\lambda)$ converges to $H_1$ as $\lambda$ converges to zero.  The lemma follows after puting $\gamma(\lambda) = A(\lambda) + B(\lambda) + C(\lambda) - H_1$.
\end{IEEEproof}
...
\]
for all positive integers $L$.  This type of result is typically called \emph{tightness} or \emph{asymptotic continuity}~\cite{Pollard_asymp_empi_proc_1989,van2009empirical,Billingsley1999_convergence_of_probability_measures}.  We ommit the proof which follows in a straightforward, but lengthy manner using an argument called \emph{symmetrisation} followed by an argument called \emph{chaining}~\cite{Pollard_asymp_empi_proc_1989,van2009empirical}.
