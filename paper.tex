...
\]
since the mean of $R_1e^{j\Phi_1}$ is 1.  By a Taylor expansion about $\lambda = 0$ we obtain
\[
q_1(\lambda) + j k_1(\lambda) = \lambda\big( j + \zeta(\lambda) \big)
\]
where $\zeta(\lambda)$ is a continuous function with $\zeta(0) = 0$.  Since $\hat{\lambda}_L$ converges almost surely to zero as $L\rightarrow\infty$ it follows that $\zeta(\hat{\lambda}_L) = o_P(1)$.  Thus
\[
...
\begin{align*}
q_2(\lambda) + j k_2(\lambda) &= \expect\big[ R_1 e^{j\fracpart{\lambda + \Phi_1}} - R_1 e^{j\fracpart{\Phi_1}}  \big] \\
&= \big(e^{j\lambda} - 1 \big) h_2(0) \\
&\hspace{0.5cm} + e^{j\lambda}\big( e^{-j\tfrac{2\pi}{M}} - 1 \big) \expect R_1 e^{j\fracpart{\Phi_1}} \chi(\Phi_1,\lambda) 
\end{align*}
since $\expect R_1 e^{j\fracpart{\Phi_1}} = h_2(0) + \expect R_1 \sin\fracpart{\Phi_1} = h_2(0)$ as a result of Lemma~\ref{lem:expectImRfracpart} and where $\chi(\Phi_1, \lambda) = \sum_{k \in \ints}\chi_k(\Phi_1, \lambda)$.
Now
\begin{align*}
 \expect R_1 e^{j\fracpart{\Phi_1}} \chi(\Phi_1,\lambda)  &= \int_{0}^{2\pi}\int_{0}^{\infty} r f(r,\phi) \chi(\phi,\lambda) dr d\phi \\
&= \sum_{k\in\ints}\int_{0}^{2\pi} g(\phi) \chi_k(\phi,\lambda) d\phi \\
&= \sum_{k=0}^{M-1}\int_{\psi_k-\lambda}^{\psi_k} g(\phi)  d\phi \\
&= \lambda \sum_{k=0}^{M-1} g\big( \phi_k(\lambda) \big) 
\end{align*}
where $\phi_k(\lambda) \in (\psi_k - \lambda, \psi_k)$ for each $k$.  The third line in the previous equation follows because the $\chi_k(\phi,\lambda)$ terms inside the integral are zero for all $\phi \in [0,2\pi]$ when $k \notin \{0,\dots,M-1\}$.  We now have
\begin{equation}\label{eq:q2k2form}
q_2(\lambda) + j k_2(\lambda) = \big(e^{j\lambda} - 1 \big) h_2(0) + B(\lambda) \sum_{k=0}^{M-1} g\big(\phi_k(\lambda)\big) 
\end{equation}
where $B(\lambda) = \lambda e^{j\lambda}\big( e^{-j\tfrac{2\pi}{M}} - 1 \big)$.  Since $\phi_k(\lambda) \rightarrow \psi_k$ as $\lambda \rightarrow 0$ and $g(\psi_k)$ is continuous at $\psi_k$ (by assumption in Theorem~\ref{thm:normality}) we have
\[
 \sum_{k=0}^{M-1} g\big(\phi_k(\hat{\lambda}_L)\big) =  \sum_{k=0}^{M-1} g(\psi_k) + o_P(1).
\]
Also $e^{j\hat{\lambda}_L} - 1 = \hat{\lambda}_L \big( j  + o_P(1) \big)$ by the argument in Lemma~\ref{lem:q1k1parts}.  Finally
\[
B(\hat{\lambda}_L) = \hat{\lambda}_L \big( o_P(1) - 2 j \sin(\tfrac{\pi}{M})\big)
\]
by Lemma~\ref{lem:Blambdaconv}.  Combining these results into~\eqref{eq:q2k2form} and taking real and imaginary parts we obtain $q_2(\hat{\lambda}_L) = \hat{\lambda}_L o_P(1)$ and 
\begin{align*}
...
\end{lemma}
\begin{proof}
A Taylor expansion about $\lambda = 0$ gives
\[
B(\lambda) = \big(  \big)\big( e^{-j\tfrac{2\pi}{M}} - 1 \big)
\]
\end{proof}
