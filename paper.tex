...
%\]
The least square estimator $\hat{\theta}$ of $\theta_0$ is the minimiser of $SS(\theta)$ under the constraint $\hat{\rho}(\theta) = \Re(Z(\theta)) > 0$.  Equivalently
\[
\hat{\theta} = \arg\max_{\theta \in [-\pi,\pi)} \Re(Z(\theta))
\]
...
\]
It follows that $q_1(\lambda) = o_P(1)$ and $k_1(\hat{\lambda}_L) = \hat{\lambda}_L\big(1 + o_P(1) \big)$
\end{lemma}
\begin{proof}
We have
\[
...
\]
since the mean of $R_1e^{j\Phi_1}$ is 1.  Thus $q_1(\lambda) = \cos(\lambda) - 1$ and $k_1(\lambda) = \sin(\lambda)$.  Now $q_1(\hat{\lambda}_L) = o_P(1)$ since $\hat{\lambda}_L$ converges almost surely to zero as $L\rightarrow\infty$.  By a Taylor expansion of $\sin(\lambda)$ about $\lambda = 0$ we obtain $k_1(\lambda) = \lambda \big( 1 + \zeta(\lambda) \big)$  where $\zeta(\lambda)$ is a continuous function with $\zeta(0) = 0$.  Thus $\zeta(\hat{\lambda}_L) = o_P(1)$ and $k_1(\hat{\lambda}_L) = \hat{\lambda}_L\big(1 + o_P(1) \big)$.
\end{proof}
...
\]
It follows that $q_2(\hat{\lambda}_L) = o_P(1)$ and $k_2(\hat{\lambda}_L) = \hat{\lambda}_L \big( H + o_P(1) \big)$ where $H$ is defined in the statement of Theorem~\ref{thm:normality}.
\end{lemma}
\begin{proof}
...
Now
\begin{align*}
q_2(\lambda) + j k_2(\lambda) &= \big(e^{j\lambda} - 1 \big) h_2(0) \\
&\hspace{0.5cm} + e^{j\lambda}\big( e^{-j\tfrac{2\pi}{M}} - 1 \big) \expect\big(R_1 e^{j\fracpart{\Phi_1}} \chi(\Phi_1,\lambda)\big)
\end{align*}
since $\expect R_1 e^{j\fracpart{\Phi_1}} = h_2(0)$ as a result of Lemma~\ref{lem:expectImRfracpart} and where $\chi(\Phi_1, \lambda) = \sum_{k \in \ints}\chi_k(\Phi_1, \lambda)$.
Now
\begin{align*}
 \expect\big(R_1 e^{j\fracpart{\Phi_1}} \chi(\Phi_1,\lambda)\big) &= \int_{0}^{2\pi}\int_{0}^{\infty} r f(r,\phi) \chi(\phi,\lambda) dr d\phi \\
&= \sum_{k\in\ints}\int_{0}^{2\pi} g(\phi) \chi_k(\phi,\lambda) d\phi \\
&= \sum_{k=0}^{M-1}\int_{\psi_k-\lambda}^{\psi_k} g(\phi)  d\phi \\
&= \lambda \sum_{k=1}^{M-1} g(\phi_k) 
\end{align*}
where for each $k$, $\phi_k \in (\psi_k-\lambda, \psi_k)$.  The third line above follows from
\end{proof}
