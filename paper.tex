...
If the functions $g$ and $g_2$ are continuous at $\tfrac{2\pi}{M}k + \tfrac{\pi}{M}$ for each $k = 0, \dots, M-1$, then the distribution of $(\sqrt{L}\hat{\lambda}_L, \sqrt{L}\hat{m}_L)$ converges to the bivariate normal with zero mean and covariance
\[
\left( \begin{array}{cc} 
...
A key assumption in Theorem~\ref{thm:consistency} is that $G(x)$ is uniquely maximised at $x = 0$ for $x \in [-\pi, \pi)$.  This assumption asserts that $G(x) \leq G(0)$ for all $x \in [-\pi, \pi)$ and that if $\{x_i\}$ is a sequence of numbers from $[-\pi,\pi)$ such that $G(x_i) \rightarrow G(0)$ as $i \rightarrow \infty$ then $x_i \rightarrow 0$ as $i \rightarrow \infty$.  Although we will not prove it here, this assumption is not only sufficient, but also necessary, for if $G(x)$ is uniquely maximised at some $x \neq 0$ then $\sfracpart{\theta_0 - \hat{\theta}}_\pi \rightarrow x$ almost surely as $L\rightarrow\infty$, while if $G(x)$ is not uniquely maximised then $\sfracpart{\theta_0 - \hat{\theta}}_\pi$ will not converge.  The next proposition describes a class of circularly symmetric distributions for which $G(x)$ is uniquely maximised at $x = 0$.  The zero mean complex Gaussian distribution with independent real and imaginary parts is in this class.  The proof is given in Appendix~\ref{appendix:prop2proof}.

\begin{proposition}\label{prop:contgg2}
Let $W = X + jY$ be a circularly symmetric complex random variable, the real and imaginary parts having continuous and differentiable joint pdf $f_{X,Y}(x,y)$ that is nonincreasing with $x^2 + y^2$.  Let $R \geq 0$ and $\Phi \in [0, 2\pi)$ be random variables satisfying $R e^{j\Phi} = 1 + W$.  Let,
\[
...
\]
for all $i \in D$.  Thus, $\sfracpart{\hat{\lambda}_L + \Phi_i} \neq -\tfrac{\pi}{M}$ for $i \in D$ when $R_i > 0$ and therefore $G_L$ is differentiable at $\hat{\lambda}_L$.  That $G_L^\prime(\hat{\lambda}_L) = 0$ follows since $\hat{\lambda}_L$ is a maximiser of $G_L$.
\end{IEEEproof}
...
\]
Since the pdf of $X$ is circularly symmetric, $f(r,\phi)$ and $g(\phi)$ are even in $\phi$ on $[-\pi,\pi]$.

Write $\cos(\phi + x)$ as a sum of even and odd parts,
\[
...
\] 
because $g(\phi) > 0$ is increasing on $[-\pi,0]$ and $\cos(\phi)$ is odd and increasing on $[-\pi,0]$.  Thus $h_1(x) = 2K \cos(x)$ is uniquely maximised at zero because $\cos(x)$ is.
\end{IEEEproof}

\begin{lemma}
Under the assumptions in Proposition~\ref{prop:contgg2} let $f_Z$ be the pdf of $\abs{W}$.  Then $z^{-1} f_Z(z)$ is differentiable and nonincreasing in $z > 0$ and the function $g(\phi) = \int_{0}^{\infty} r f(r,\phi) dr$ is symmetric and unimodal with mode at $\phi = 0$.
\end{lemma}
We have
\[
f_{X,Y}(x,y) = \frac{f_Z(\sqrt{x^2 + y^2})}{\sqrt{x^2 + y^2}}.
\]
Put $z = \sqrt{x^2 + y^2}$ and, since $X$ is circularly symmetric,
\[
f_{X,Y}(x,y) = f_{X,Y}(z,0) = \frac{f_Z(z)}{z}.
\]
Since $f_{X,Y}(z,0)$ is differentiable and nonincreasing in $z = \sqrt{x^2 + y^2}$ (by assumption in the proposition statement) we have that $z^{-1} f_Z(z)$ is differentiable and nonincreasing in $z > 0$.  So the derivative $\frac{d}{dz} \big( z^{-1} f_Z(z) \big) \geq 0$ for all $z > 0$.   Using~\eqref{eq:pdfRPhi},
\[
...
\]
and since the term inside the integral is always positive,
