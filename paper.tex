...
\[
\sqrt{L} Q(\hat{\lambda}_L) = \sqrt{L} \hat{\lambda}_L\big( ph_1(0) + ?  + o_P(1) \big)
\]
\end{lemma}
\begin{IEEEproof}
Write
...
&= \lambda \big( h_1(0) \; + \; \zeta(\lambda) K \big),
\end{align*}
where we put $K = \prob R_1$, a finite constant.  Because $\hat{\lambda}_L$ converges almost surely to zero as $L \rightarrow\infty$, then $\zeta(\hat{\lambda}_L)$ converges almost surely (and therefore also in probability) to zero as $L \rightarrow\infty$, i.e. $\zeta(\hat{\lambda}_L) = o_P(1)$.  So, 
\begin{align*}
k_1(\hat{\lambda}_L) &= \hat{\lambda}_L \big( h_1(0) \; + \; \zeta(\hat{\lambda}_L) K \big) \\
&= \hat{\lambda}_L \big( h_1(0)  + o_P(1)\big).
\end{align*}
\end{IEEEproof}
