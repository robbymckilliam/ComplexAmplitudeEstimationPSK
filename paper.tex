...
We are interested in analysing $\hat{\theta}$, the maximiser of $\Re(Z(\theta))^2$.  Recalling the definition of $R_i$ and $\Phi_i$ from the statement of Theorem~\ref{thm:consistency},
\begin{align*}
y_i &= a_0 s_i + w_i \\
...
Put $G_L(\lambda) = \frac{1}{\rho_0}\Re(Z(\theta))$ and observe that
\[
...
BLERG.  It follows after a bit of discussion and the use of Lemma~\ref{lem:Zconunif} that 
\[
\hat{\rho} = \rho_0 G_L(\lambda_L) \rightarrow \rho_0 G(0). 
\] 
\end{IEEEproof}
...
\[
\prob\{\abs{G_L(\lambda) - G(\lambda)} > \epsilon \} = O(e^{-\epsilon^2 L}).
\]
\end{lemma}
\begin{IEEEproof}
\end{IEEEproof}
...
\[
\sqrt{L} \prob G^\prime_L(\lambda_L) = \sqrt{L} \lambda_L( G(0)  + o_P(1) )
\]
and Lemma~\ref{lem:empiricprocc} shows that
\[ 
\sqrt{L} \left( G_L^\prime(\lambda_L) - \prob G_L^\prime(\lambda_L) \right) = \sqrt{L} G_L'(0) + \sqrt{L}\lambda_L( ? +  o_P(1) )
\]
It follows that
\[
\sqrt{\lambda_L} = o_P(1) + \frac{\sqrt{L}G_L'(0)}{G(0)  + o_P(1)}.
\]
Observe that 
\[
\sqrt{L} G_L'(0) = \frac{1}{\sqrt{L}} \sum_{i \in P} R_i \sin(\lambda + \Phi_i) + \frac{1}{\sqrt{L}} \sum_{i \in D} R_i \sin\sfracpart{\lambda + \Phi_i}.
\]
