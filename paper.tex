...
X_L(\lambda) = \frac{1}{\sqrt{L}} \sum_{i \in P \cup D} g_i f_i(\lambda, R_i, \Phi_i),
\end{equation}
...
\expect \sup_{\sabs{\lambda} < \epsilon} \abs{ X_L(\lambda) } < \kappa,
\]
where $X_L(\lambda)$ is defined in~\eqref{eq:ZpsiCondGaussProc}.
\end{lemma}
\begin{IEEEproof}
...
\expect_{R,\Phi} \sup_{\sabs{\lambda} < \epsilon} \abs{ X_L(\psibf) } \leq K_1 \sqrt{\frac{C_\epsilon}{L}}
\end{equation}
where $K_1$ is a finite, positive constant, and
\[
C_\epsilon = 2\epsilon\sum_{i\in P \cup D}R_i^2 + 4\sum_{i\in P \cup D}R_i^2 I_{\epsilon}(\Phi_i)
\]
where $I_{\epsilon}(\Phi_i)$ returns one when $\Phi_i \in \cup_{k\in\ints}[\psi_k - \epsilon, \psi_k + \epsilon]$ and zero otherwise.  Now,
\[
\tfrac{1}{L}\expect C_\epsilon =  2\epsilon\expect R_1^2 + 4 \expect R_i^2 I_{\epsilon}(\Phi_i).
\]
...
\end{align*}
where $o(1)$ goes to zero as $\epsilon$ goes to zero, by the mean value theorem.  Thus $\expect R_i^2 I_\epsilon(\Phi_i) \leq K_2\epsilon$ for some constant $K_2$. Since $\sqrt{\cdot}$ is a concave function on the positive real line, and since $C_{\epsilon}$ is always positive, then
\[
\expect \sqrt{\frac{C_\epsilon}{L}} \leq  \sqrt{\tfrac{1}{L}\expect  C_\epsilon} < \sqrt{\epsilon( K_2 + 2\expect R_1^2)  }
\]
by Jensen's inequality.  Applying $\expect$ to both sides of~\eqref{eq:supZCK1},
\[
\expect \sup_{\sabs{\lambda} < \epsilon} \abs{ X_L(\psibf) } \leq K_1 \sqrt{\tfrac{1}{L}\expect C_\epsilon} < K_1 \sqrt{\epsilon( K_2 + 2\expect R_1^2)}.
\]
Choosing 
\[
...
\[
\expect_{R,\Phi} \sup_{\sabs{\lambda} < \epsilon} \abs{ X_L(\lambda) } \leq K_1 \sqrt{\frac{C_\epsilon}{L}}.
\]
\end{lemma}
\begin{IEEEproof}
...
\[
d_1(\lambda, \lambda_1) = \sum_{i \in P \cup D} \big( f_{i}(\lambda) - f_{i}(\lambda_1) \big)^2 \leq 2^{-k} C_\epsilon,
\]
and the psuedometric
\[
d_2(\lambda, \lambda_2) = \sum_{i \in P \cup D} \sabs{f_{i}(\lambda) - f_{i}(\lambda_2)} \leq 2^{-k} D_\epsilon
\]
where 
\[
D_\epsilon = \epsilon\sum_{i\in P \cup D}R_i + \sum_{i\in P \cup D}R_i I_{\epsilon}(\Phi_i).
\]
We specifically define $T_\epsilon(0)$ to contain only zero.  Defined this way, $T_\epsilon(0)$ satisfies the inequality above because both $d_1(\lambda, 0)$ and $d_2(\lambda, 0)$ are less than $C_\epsilon$ for all $\lambda \in B_\epsilon$, as a result of Lemma~\ref{lem:epslmlemma}.  The existance of $T_\epsilon(k)$ for every positive integer $k$ will be proved by construction in Lemma~\ref{}.

Lemma~\ref{lem:limexpTk} shows that
\[
\lim_{k\rightarrow\infty}\expect_{R,\Phi}\sup_{\lambda \in T_\epsilon(k)} \sabs{X_L(\lambda)} = \expect_{R,\Phi} \sup_{\sabs{\lambda} < \epsilon} \sabs{X_L(\lambda)}.
\]
Lemma~\ref{lem:chaining3} shows that
\[
\expect_{R,\Phi} \sup_{\lambda \in T_\epsilon(k)} \sabs{X_L(\lambda)} \leq \sqrt{\frac{C_\epsilon}{L}} \sum_{i=1}^{k} \frac{?}{2^{i/2}}
\]
The sum above converges as $k\rightarrow\infty$ so lemma holds with
\[
K_1 = \sum_{i=1}^{\infty} \frac{?}{2^{i/2}}.
\]
\end{IEEEproof}

\begin{lemma}\label{lem:limexpTk}
\[
\lim_{k\rightarrow\infty}\expect_{R,\Phi}\sup_{\lambda \in T_\epsilon(k)} \sabs{X_L(\lambda)} = \expect_{R,\Phi} \sup_{\sabs{\lambda} < \epsilon} \sabs{X_L(\lambda)}
\]
\end{lemma}
\begin{IEEEproof}
Let $b_k(\lambda)$ be a function that maps $\lambda \in B_\epsilon$ to a point in $T_\epsilon(k)$ such that  $d_2(\lambda,b_k(\lambda)) \leq 2^{-k}D_\epsilon$.  The existance of $b_k$ is guaranteed by the definition of $T_\epsilon(k-1)$.  So, for all $\lambda \in B_\epsilon$,
\begin{align*}
\sabs{X_L(\lambda) - X_L(b_k(\lambda))} &= \abs{\frac{1}{\sqrt{L}}\sum_{i\in P \cup D}g_i(f_i(\lambda) - f_i(b_k(\lambda)))} \\
&\leq  \frac{1}{\sqrt{L}}\sum_{i\in P \cup D}\abs{g_i}\sabs{f_i(\lambda) - f_i(b_k(\lambda))} \\
&\leq \frac{g_\text{max}}{\sqrt{L}} \sum_{i\in P \cup D}\sabs{f_i(\lambda) - f_i(b_k(\lambda))} \\
&\leq \frac{g_\text{max}}{\sqrt{L}} d_2(\lambda,b_k(\lambda)) \\
&\leq \frac{g_\text{max}}{\sqrt{L}} 2^{-k} D_\epsilon.
\end{align*}
where $g_\text{max} = \max_{i\in P\cup D} \abs{g_i}$. The bound applies uniformly over $\lambda \in B_\epsilon$, so
\[
\sup_{\sabs{\lambda} < \epsilon} \sabs{X_L(\lambda) - X_L(b_k(\lambda))} \leq \frac{g_\text{max}}{\sqrt{L}} 2^{-k} D_\epsilon.
\]
Applying $\expect_{R,\Phi}$ to both sides of this inequality,
\[
\expect_{R,\Phi} \sup_{\sabs{\lambda} < \epsilon} \sabs{X_L(\lambda) - X_L(b_k(\lambda))} \leq K_4 2^{-k} D_\epsilon
\]
where $K_4 = \tfrac{1}{\sqrt{L}}\expect_{R,\Phi} g_\text{max}$ is a positive constant as a result of Lemma~\ref{} (BLERG, a maximal inequality lemma is needed).  Because $T_\epsilon(k)$ is a subset of $B_\epsilon$,
\[
\sup_{\lambda \in T_\epsilon(k)} \expect_{R,\Phi} \sabs{X_L(\lambda)} \leq \sup_{\sabs{\lambda} < \epsilon} \expect_{R,\Phi} \sabs{X_L(\lambda)},
\]
and so,
\begin{align*}
0 &\leq \sup_{\sabs{\lambda} < \epsilon} \expect_{R,\Phi} \sabs{X_L(\lambda)} - \sup_{\lambda \in T_\epsilon(k)} \expect_{R,\Phi} \sabs{X_L(\lambda)} \\
&\leq \sup_{\sabs{\lambda} < \epsilon} \expect_{R,\Phi} \big( \sabs{X_L(\lambda)} - \sabs{X_L(b_k(\lambda))} \big) \\
&\leq \sup_{\sabs{\lambda} < \epsilon} \expect_{R,\Phi} \sabs{X_L(\lambda) - X_L(b_k(\lambda))} \\
&\leq K_4 2^{-k} D_\epsilon.
\end{align*}
The lemma follows since this converges to zero as $k\rightarrow\infty$.
\end{IEEEproof}
...
\[
\expect_{R,\Phi} \sup_{\psibf \in T_\epsilon(k) } \abs{ X_L(\lambda) } \leq \sqrt{C_\epsilon} \sum_{i=1}^{k}\frac{?}{2^{i/2}}.
\]
\end{lemma}
\begin{IEEEproof}
Let $b_k(\lambda)$ maps each $\lambda \in T_\epsilon(k)$ to an element in $T_\epsilon(k-1)$ such that $d(\lambda, b_k(\lambda)) \leq 2^{1-k}C_\epsilon$. 
\end{IEEEproof}
