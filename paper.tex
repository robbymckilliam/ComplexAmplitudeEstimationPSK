...
\end{lemma}
\begin{IEEEproof}
Observe that $G_L$ is differentiable at all points $\lambda \in [-\pi , \pi)$ those which staisfy $\fracpart{\lambda + \Phi_i} = -\tfrac{\pi}{M}$ for any $i = P \cup D$.  Lemma~\ref{lem:fracpartlambdahatnotpi} shows that,
\[
\sabs{\sfracpart{\hat{\lambda}_L + \Phi_i}} \leq \frac{\pi}{2M} - \frac{\pi}{2LM ?}
\]
for all $i \in P \cup D$.  The proof follows since $y_i = 0$ with probability zero.  
\end{IEEEproof}
...
\end{lemma}
\begin{IEEEproof}
Recall that $\hat{d}_i = \hat{d}_i(\hat{\theta})$ are the minimsers of $SS$.  The proof now proceeds by contradiction.  Assume that $\sfracpart{\hat{\lambda}_L + \Phi_i} > \frac{1}{2M} - \frac{1}{2LM ?}$ for some $k$.  Put $r_i = \hat{d}_i e_k$, then
\begin{align*}
...
R &\geq  \frac{2}{L^2}\abs{\hat{Y} y_k}\Re\left(\eta e^{j(\phi_i - \hat{\theta})}\hat{d}_k^* \right) +  \frac{1}{L^3}\abs{ \eta}^2 \abs{ y_k }^2 \\
 ...
 \]
 Unfinished, but this proof will work.  BLERG.
\end{IEEEproof}
