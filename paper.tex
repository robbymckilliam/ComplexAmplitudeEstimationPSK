...
&= \lambda a_k(\lambda) + \lambda b_k(\lambda) + \int_{\psi_{k} - \lambda}^{\psi_{k}} s_k(\phi) g(\phi) d\phi,
\end{align*}
where, for each integer $k = 1, \dots, M-1$, the functions $a_k$ and $b_k$ are
...
\]
Observe that $a_k(\lambda)$ and $b_k(\lambda)$ are functions that go to zero as $\lambda$ goes to zero.
Now, when $k = 0$, we have $\psi_{-1} < 0 < \psi_0 - \lambda < \psi_0 < 2\pi$, so that
\begin{align*}
\int_0^{2\pi} v_0(\lambda, \phi) &g(\phi) d\phi = \int_{0}^{\psi_{k}} v_0(\lambda, \phi) g(\phi) d\phi \\
&= \lambda a_0(\lambda) + \lambda b_0(\lambda) + \int_{\psi_{0} - \lambda}^{\psi_{0}} s_0(\phi) g(\phi) d\phi,
\end{align*}
where $a_0(\lambda)$ is defined as in~\eqref{eq:ak} and 
\[
b_0(\lambda) = \zeta_0(\lambda) \int_{0}^{\psi_k-\lambda} g(\phi) d\phi.
\]
When $k = M$, we have $0 < \psi_{M-1} < 2\pi < \psi_{M} - \lambda$, so that
\begin{align*}
\int_0^{2\pi} v_{M}(\lambda, \phi) g(\phi) d\phi &= \int_{\psi_{M-1}}^{2\pi} v_{M}(\lambda, \phi) g(\phi) d\phi \\
&= \lambda b_{M}(\lambda)
\end{align*}
where 
\[
...
&= \sum_{k=0}^{M-1}\int_{\psi_{k} - \lambda}^{\psi_{k}} s_k(\phi)g(\phi)d\phi + \sum_{k=0}^{M-1} \lambda a_k(\lambda) + \sum_{k=0}^{M} \lambda b_k(\lambda) \\
