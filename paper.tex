...
Put 
\[
Z_L(\lambda) = \sqrt{L}\big(G_L^\prime(\lambda) - \expect G_L^\prime(\lambda) - G_L^\prime(0) + \expect G_L^\prime(0)\big)
\] 
so that
\[
\sqrt{L}\big( G_L^\prime(\hat{\lambda}_L) - Q(\hat{\lambda}_L) \big) = Z_L(\hat{\lambda}_L) + \sqrt{L} G_L^\prime(0)
\]
