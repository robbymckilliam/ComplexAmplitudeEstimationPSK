...
\begin{lemma}
Let $W$ be a circularly symmetric random variable with constant magnitude $\abs{W} = c > 0$ and phase uniformly distributed on $[-\pi,pi)$.  Let $R \geq 0$ and $\Phi$ be real random variables such that $Re^{-j2\pi \Phi} = 1 + W$.  Then the pdf of $\Phi$ is
\[
