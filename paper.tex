...
We use $\sfracpart{x}$ to denote $x$ taken `modulo $2\pi$' into the interval $[-\pi, \pi)$, that is
\[
\fracpart{x} = x - 2\pi\operatorname{round}\left(\frac{x}{2\pi}\right).
\]
where $\round(\cdot)$ takes its argument to the nearest integer.  The direction of rounding for half-integers is not important so long as it is consistent.  We have chosen to round up half-integers here.
...
\item Put $\lambda_L = \sfracpart{\theta_0 - \hat{\theta}}$ and $m_L = \hat{\rho} - \rho_0(p + dh)$.  Then the vector $[\sqrt{L}\lambda_L, \sqrt{L}m_L]$ converges to the bivariate normal distribution with zero mean and covariance
\[
C = ?
\]
