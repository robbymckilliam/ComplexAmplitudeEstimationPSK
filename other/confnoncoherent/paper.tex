...
It will become apparent why $\langle\theta_0 - \hat{\theta}\rangle^2$ rather than $(\theta_0 - \hat{\theta})^2$ is the appropriate measure of error for the phase parameter.
...
The theorems place conditions on $\sfracpart{\theta_0 - \hat{\theta}}$ rather than directly on $\theta_0 - \hat{\theta}$.  This makes practical sense, and to see why let $s_i' = e^{j2\pi k/M}s_i$ be $M$-PSK symbols obtained by rotating $s_1, \dots, s_L$ by $e^{j2\pi k/M}$ for some integer $k$.  Then
\[
