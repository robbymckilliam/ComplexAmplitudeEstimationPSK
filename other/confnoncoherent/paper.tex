...
\[
f(r,\phi) = \frac{r f_Z(\sqrt{\sabs{r^2 - 2r\cos\phi + 1}})}{2\pi\sqrt{\sabs{r^2 - 2r\cos\phi + 1}}}.
\]
...
Figures~\ref{fig:plotphaseM2}~and~\ref{fig:plotphaseM4} show the mean square error (MSE) of the phase estimator $\hat{\theta}$ computed as $\tfrac{1}{T}\sum_{i=1}^T\sfracpart{\hat{\theta}_i - \theta_0}^2$.  The dots show the simulated MSE of the least square estimator.  The dashed line is the simulated MSE of the unmodulated carrier estimator~\eqref{eq:hataumod} that has a priori knowledge of the transmitted symbols $s_1, \dots, s_L$.  %The dashed line acts as a lower bound on performance.  
