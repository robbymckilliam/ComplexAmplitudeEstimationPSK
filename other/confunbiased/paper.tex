...
%opening
\title{Unbiased noncoherent amplitude estimation}
...
\begin{abstract}
We consider the problem of estimating the amplitude of communications signal when the observations are pertubed by additive white Gaussian noise.  We focus on signaling constellations that have symbols evenly distributed on the complex unit circle, i.e., $M$-ary phase shift keying.  Recently, the asymptotic properties of the maximum likelihood estimator of the amplitude were derived.  The estimator is asymptotically biased, and the bias is large when the signal to noise ratio is small.  We consider a method for correcting this bias.  This involves the reciever observing a period of `silence' which is used to estimate the statistical properties (i.e. the variance) of the noise.  This approach strongly hinges on the assumption that the underlying noise distribution is indeed Gaussian.  We discuss how this approach could be extended to handle the situation when the distribution of the noise is not assumed.
\end{abstract}
\begin{keywords}
