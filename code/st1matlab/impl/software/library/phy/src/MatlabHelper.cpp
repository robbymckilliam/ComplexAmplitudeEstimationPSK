...
void CopyMatlabComplexToVector(const mxArray *src, std::vector<complexT>& v){
  unsigned int N = mxGetNumberOfElements(src);
  if( v.size() != N ) v.resize(N, complexT(0,0));
  double* r = (double*) mxGetData(src);
  double* i = (double*) mxGetImagData(src);
  if(i != NULL) for(unsigned int n = 0; n < N; n++) v[n] = complexT(r[n],i[n]); 
  else for(unsigned int n = 0; n < N; n++) v[n] = complexT(r[n],0.0);
}
...
std::vector<complexT> GetComplexArray(const mxArray *src)
{
  ...
  switch(mxGetClassID(src))
  {
    case mxDOUBLE_CLASS:
      ...
      if(di != NULL) for(unsigned int n = 0; n < N; n++) ret[n] = complexT(dr[n],di[n]); 
      else  for(unsigned int n = 0; n < N; n++) ret[n] = complexT(dr[n],0.0);
      break;
    case mxSINGLE_CLASS:
      fr = (float*) mxGetData(src);
      fi = (float*) mxGetImagData(src);
      if(fi != NULL) for(unsigned int n = 0; n < N; n++) ret[n] = complexT(fr[n],fi[n]); 
      else  for(unsigned int n = 0; n < N; n++) ret[n] = complexT(fr[n],0.0);
      break;
  }
