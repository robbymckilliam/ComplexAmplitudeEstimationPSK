...
% dcblock.m 
%
% [RxSampFilt] = dcblockST1UL(RxSamp)
%
...
% transmitted in the ST1 uplink waveform for frequency estimation purposes.
% Once the frequency has been estimated, the discrete tone should be
% removed in order to avoid bit error rate degradations. The implementation
% uses an order-one IIR filter.
% 
% Inputs:
...
function [RxSampFilt] = dcblockST1UL(RxSamp)
    
% ST1 UL: coefficents of order-one IIR filter

p = 0.999501949180559; % coeff for cut-on frequency at 3Hz
    % fc = 3 / (OS/(2*Ts)); % normalised cut-on frequency
