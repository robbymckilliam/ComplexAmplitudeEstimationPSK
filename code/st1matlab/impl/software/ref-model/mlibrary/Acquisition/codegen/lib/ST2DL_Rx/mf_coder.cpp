              ...
              emxArray_creal_T *mfout)
{
  ...
  /* q = acqparam.OS; */
  /* T = acqparam.T; */
  /* 'mf_coder:27' Ts = p*T/q; */
  Ts = mrdivide(T, q);

  /* 'mf_coder:28' mini = min(min(P),min(D)); */
  ixstart = 1;
  mtmp = P[0];
  if (rtIsNaN(P[0])) {
    nm1d2 = 2;
  ...
  /* 'mf_coder:29' maxi = max(max(P),max(D)); */
  ixstart = 1;
  mtmp = P[0];
  if (rtIsNaN(P[0])) {
    nm1d2 = 2;
  ...
  /* 'mf_coder:30' N = length(rx); */
  /* 'mf_coder:31' L = maxi; */
  /* 'mf_coder:33' ns = (p*q*mini - p*N : p*q*maxi - p)*(-1)*Ts/p - tauhat; */
  y = q * mini;
  b_y = q * maxi;
  mtmp = q * mini - 16000.0;
  ndbl = q * maxi - 1.0;
  ...
  /* 'mf_coder:35' lvec = conj( rrcpulse(ns,rolloff,T,q) .*... */
  /* 'mf_coder:36'     (abs(ns) <= RrcNsym*q*Ts)); */
  ...
  /* 'rrcpulse:18' tol = 2*eps; */
  /* 'rrcpulse:19' idx1 = (abs(t) < tol); */
  c_abs(ns, r10);
  ...
  /* 'rrcpulse:21' idx2 = (abs(abs(8*rolloff*fc*t) - 1.0) < tol); */
  y = 8.0 * rolloff * apnd;
  ...
  /* 'rrcpulse:25' rrc(idx1) = -sqrt(2.*fc) ./ (pi.*fs) .* (pi.*(rolloff-1) - 4.*rolloff ); */
  ...
  /* 'rrcpulse:26' rrc(idx2) = sqrt(2.*fc) ./ (2.*pi.*fs) ... */
  /* 'rrcpulse:27' 		* (    pi.*(rolloff+1)  .* sin(pi.*(rolloff+1)./(4.*rolloff)) ... */
  /* 'rrcpulse:28' 		- 4.*rolloff .* sin(pi.*(rolloff-1)./(4.*rolloff)) ... */
  /* 'rrcpulse:29' 		+ pi.*(rolloff-1) .* cos(pi.*(rolloff-1)./(4.*rolloff)) ... */
  /* 'rrcpulse:30' 		); */
  ...
  /* 'rrcpulse:31' rrc(idx3) = -4.*rolloff./fs .* ( cos((1+rolloff).*2.*pi.*fc.*t(idx3)) + ... */
  /* 'rrcpulse:32'         sin((1-rolloff).*2.*pi.*fc.*t(idx3)) ./ (8.*rolloff.*fc.*t(idx3)) ) ... */
  /* 'rrcpulse:33'         ./ (pi .* sqrt(1./(2.*fc)) .* ((8.*rolloff.*fc.*t(idx3)).^2 - 1)); */
  ...
  /* 'mf_coder:37' zvec = zeros(1,p*N - p + 1) + 1j*zeros(1,p*N - p + 1); */
  for (i7 = 0; i7 < 16000; i7++) {
    zvec[i7].re = 0.0;
    zvec[i7].im = 0.0;

    /* 'mf_coder:38' zvec(1:p:p*N-p+1) = rx; */
    zvec[i7] = rx[i7];
  }

  /* zerofilled r */
  /* 'mf_coder:39' h = conv_fft(lvec, zvec); */
  ...
  /* 'conv_fft:10' P = numel(a); */
  /* 'conv_fft:11' Q = numel(b); */
  /* 'conv_fft:12' L = P + Q - 1; */
  /* 'conv_fft:13' M = min([P,Q]); */
  ...
  /* 'conv_fft:15' c = ifft(fft(a,L) .* fft(b,L)); */
  i7 = b_lvec->size[0];
  ...
  /* length(h) */
  /* length(q*(mini:maxi)) */
  /* 'mf_coder:45' mfout = h(q*(mini:maxi) - q*mini + 1)*Ts; */
