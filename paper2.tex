...
\]
The lemma follows since $C_L$ and $D_L$ are indpendent.
\end{IEEEproof}
...
so
 \begin{align*}
 \vert &f_{i}(\lambda) - f_{i}(\lambda_2)\vert \\
...
Thus,
\[
\sum_{i \in D, i \notin S} \sabs{f_{i}(\lambda) - f_{i}(\lambda^*)} < \epsilon 2^{-k} \sum_{i \in D, i \notin S} R_i \leq \epsilon 2^{-k} \sum_{i \in D} R_i
\]

To handle the case when $i \in S$ we need to add some extra elements to the set $U$.  Let $\sigma$ be a permutation of the indices in $S$ such that $\fracpart{\Phi_i}$ are in ascending order.
