...
\]  
Observe that for any real number $\phi$, the pdf of $X$ and $e^{j\phi}X$ are the same, that is, the pdf is invariant to phase rotation.  If $\expect\abs{X} = \expect Z$ is finite, then $X$ has zero mean because
\begin{align*}
...
\end{equation}
where $\Im(\cdot)$ denotes the imaginary part.  The next result will be useful for the analysis of the least squares estimator.  %Me ommit the proof, which is straightforward, but tedious.

\begin{proposition}\label{lem:gg2cont}
Let $X = Z e^{j\Theta}$ be a circularly symmetric complex random variable with pdf $\tfrac{1}{2\pi}f_Z(z)$ that is continuous on the positive reals and with $\expect \abs{X}^2 = \expect Z^2$ finite.  Let $R > 0$ and $\Phi \in [0, 2\pi)$ be random variables satisfying $R e^{j\Phi} = 1 + X$ and with joint pdf $f(r,\phi)$.  Then
\[
...
\]
are continuous functions from $\reals$ to $\reals$.
\end{proposition}
\begin{IEEEproof}
From~\eqref{eq:pdfRPhi},
\[
g(\phi) = \int_{0}^\infty \frac{r^2 f_Z(\sqrt{r^2 - 2r\cos\phi + 1})}{\sqrt{r^2 - 2r\cos\phi + 1}} dr,
\]
and
\[
g_2(\phi) = \int_{0}^\infty \frac{r^3 f_Z(\sqrt{r^2 - 2r\cos\phi + 1})}{\sqrt{r^2 - 2r\cos\phi + 1}} dr.
\]
The continuity of these functions follows immediately from the continuity of $f_Z$.
\end{IEEEproof}
...
\begin{proposition}
Let $X = Z e^{j\Theta}$ be a circularly symmetric complex random variable with pdf $\tfrac{1}{2\pi}f_Z(z)$ that is continuous and differentiable on the positive reals.  Let $R > 0$ and $\Phi \in [0, 2\pi)$ be random variables satisfying $R e^{j\Phi} = 1 + X$.  Let,
\[
h_1(x) = \expect R \cos(x + \Phi) \;\;\; \text{and} \;\;\; h_2(x) =  \expect R \cos\sfracpart{x + \Phi}.
\]
If $z^{-1} f_Z(z)$ is nonincreasing with $z$, then $\abs{h_1(x)}$ is uniquely maximised at $x=0$ over the interval $[-\pi,\pi]$ and $\abs{h_2(x)}$ is uniquely maximised at $x = 0$ over the interval $[-\tfrac{\pi}{M},\tfrac{\pi}{M}]$.
\end{proposition}
...
\]
as $L \rightarrow \infty$.  It remains to prove the lemmas that we have used.  These lemmas are proved in~\cite{McKilliam_leastsqPSKpilotsdata_2012appendix}.
